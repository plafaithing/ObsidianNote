...
components. We implement these ideas in Cordis, a meta-framework of spatiotemporal com-
posability that provides a core library with effect tracking and coeffect resolution, as well as a
...
To characterize the requirements of dynamic composition, we identify two orthogonal dimen-
sions beyond the well-studied algebraic aspects of composition:
...
verifiable manner. This requires managing dependency topology and coordinating com-
ponent lifecycles in response to dependency changes.
...
contain executable code1 and will therefore require such a restart upon removal. Although
...
Spatial limitation. VSCode does provide extensionDependencies for declaring dependen-
cies between extensions, but it sees little use: among the top 100 extensions by install count,
only 7 declare extensionDependencies on non-built-in extensions.1 This scarcity reflects the
shape of the extension API, which exposes fixed, surface-level extension points such as com-
mands, views, and language features. Extensions contribute to the host through these points
...
session state and persistence, provide context management and memory systems [12], orches-
trate subagents and multi-agent workflows [13], and expose interfaces to users and automation.
A future harness may generate and deploy modifications to its own components while contin-
uously serving requests. Model-synthesized reusable tools provide a narrower precursor to
...
[3] yield spatial composability at the granularity of a service. In practice, most software toler-
ates the lack of fine-grained composability by deferring to these coarse-grained mechanisms:
...
process-local accumulated state (e.g., caches, connections, partial computations), and rebuild-
ing it takes seconds to minutes [15]; maintaining availability in the interim requires redundant
replicas, incurring resource overhead to compensate for the inability to recover a single
component. Spatially, container-level orchestration cannot express dependencies between com-
ponents sharing an address space, and introduces network overhead for interactions that could
...
about environmental modifications, and coeffects for reasoning about environmental require-
ments. However, existing formulations restrict reasoning to compile-time analysis over lexically
...
explicit inverse that the runtime tracks, and both tracking and recovery preserve com-
position, so the context is recovered upon component removal. This establishes local
...
composability.
3. We unify the effect context and the coeffect context into a single context type (Sec-
tion 3.3), in which an observational equivalence on the coeffects supplies the effects with
...
Wadler [23] popularized the approach in Haskell. A monad (𝑇,𝜂,𝜇) on a category 𝒞︀ encap-
sulates an effectful computation as a value of type 𝑇(𝐴), with 𝜂 : 𝐴 → 𝑇(𝐴) lifting pure
...
monads, establishing a framework in which effect interfaces are decoupled from their imple-
mentations. An effect signature Σ declares a set of operations (e.g., get : () → 𝑆, put : 𝑆 → ()
...
captures context-dependent computation: 𝜀 : 𝐷(𝐴) → 𝐴 extracts the current value from a con-
text, and 𝛿 : 𝐷(𝐴) → 𝐷(𝐷(𝐴)) duplicates context for nested access. The Environment comonad
...
tracking, sensitivity analysis [34], and information-flow control [35, 36] within a unified alge-
braic framework [37].
...
describe how it depends on its environment. These two directions correspond to the two dimen-
sions of dynamic composability identified in Section 1:
• Temporal composability demands that a component’s modifications to the shared envi-
ronment be revertible upon unloading. The relevant effects are the stateful ones, which
...
managed reactively. Such dependencies are the very thing coeffects capture, and manag-
ing them amounts to resolving each against what the environment supplies.
...
operate on them directly, establishing dynamically the guarantees these systems provide stat-
ically.
8

3. Revertible Effects and Reactive Coeffects
This section lifts the concepts of effects and coeffects introduced in Section 2 to runtime mecha-
nisms, constructing a theory of dynamic composition. The central idea is to turn the typing
...
Given any impure function 𝑓 : 𝑋 → 𝑌, we transform it into a pure form 𝑓 : Γ×𝑋 → Γ×
impure
𝑌, where Γ is the context and all possible side effects can be represented as transformations
on Γ. For any fixed input 𝑥 : 𝑋, the induced map 𝛾 ↦ pr (𝑓(𝛾,𝑥)) captures the side effect of 𝑓
1
...
• Identity: id , the identity function on Γ, acts as the unit of composition.
Γ
To model effects that can be undone, we pair each transformation 𝑓 with another transfor-
mation 𝑔 that undoes 𝑓, and call 𝑔 a left inverse of 𝑓, abbreviated to inverse throughout the paper.
...
order. It makes (Γ → Γ)×(Γ → Γ) a monoid with unit (id ,id ), the product of the monoid of
Γ Γ
transformations with its opposite, which we call the twisted composition monoid 𝔗 over Γ.
Γ
...
In particular, the initial effect context can be represented as (𝛾 ,id ).
|                   |     |          |     |     |                              |     |     | 0   | Γ   |     |
| ----------------- | --- | -------- | --- | --- | ---------------------------- | --- | --- | --- | --- | --- |
...
Γ
(6)
|     | track |     | =   | (𝑓,𝑔) |     | ↦   | (𝛾,𝜑) | ↦ (𝑓(𝛾),𝜑∘𝑔) |     |     |
| --- | ----- | --- | --- | ----- | --- | --- | ----- | ------------ | --- | --- |
Γ
This transformation converts a forward function 𝑓 together with a candidate inverse 𝑔 into
a transformation of the effect context 𝜕Γ. Applying track (𝑓,𝑔) to a state (𝛾,𝜑) transforms 𝛾
Γ
...
| Proof. For all (𝛾,𝜑) |     | ∈ 𝜕Γ: |     |        |             |     |                 |     |     |     |
| -------------------- | --- | ----- | --- | ------ | ----------- | --- | --------------- | --- | --- | --- |
|                      |     |       | (pr | ∘track | (𝑓,𝑔))(𝛾,𝜑) |     | = pr (𝑓(𝛾),𝜑∘𝑔) |     |     |     |
|                      |     |       | 1   |        | Γ           |     | 1               |     |     |     |
= 𝑓(𝛾)
|     |     |     |     |     |     |     | = (𝑓 ∘pr | )(𝛾,𝜑) |     | □   |
| --- | --- | --- | --- | --- | --- | --- | -------- | ------ | --- | --- |
1
Theorem 5. track  is a monoid homomorphism from 𝔗  into 𝜕Γ → 𝜕Γ. That is,
|               | Γ       |         |      |         |     |            | Γ   |            |         |     |
| ------------- | ------- | ------- | ---- | ------- | --- | ---------- | --- | ---------- | ------- | --- |
| 1. track      | (id ,id | ) =     | id ; |         |     |            |     |            |         |     |
| Γ             | Γ       | Γ       | 𝜕Γ   |         |     |            |     |            |         |     |
| 2. for all (𝑓 | ,𝑔      | ),(𝑓 ,𝑔 | ) ∈  | 𝔗 ,     |     |            |     |            |         |     |
|               | 1 1     | 2       | 2    | Γ       |     |            |     |            |         |     |
|               |         | track   | ((𝑓  | ,𝑔 )∘(𝑓 | ,𝑔  | )) = track | (𝑓  | ,𝑔 )∘track | (𝑓 ,𝑔 ) | (8) |
|               |         |         | Γ    | 1 1     | 2   | 2          | Γ   | 1 1        | Γ 2 2   |     |
Proof.
1. The unit is carried to the unit, since track (id ,id )(𝛾,𝜑) = (𝛾,𝜑∘id ) = (𝛾,𝜑).
|                                           |     |     |     |     |     | Γ     | Γ Γ |     | Γ   |     |
| ----------------------------------------- | --- | --- | --- | --- | --- | ----- | --- | --- | --- | --- |
| 2. For the multiplication, take any (𝛾,𝜑) |     |     |     |     |     | ∈ 𝜕Γ: |     |     |     |     |
10

|                                                 | (track | (𝑓  | ,𝑔 )∘track |     | (𝑓  | ,𝑔 ))(𝛾,𝜑) | =       | track | (𝑓 ,𝑔    | )(𝑓 (𝛾),𝜑∘𝑔 | )         |     |
| ----------------------------------------------- | ------ | --- | ---------- | --- | --- | ---------- | ------- | ----- | -------- | ----------- | --------- | --- |
|                                                 |        | Γ   | 1 1        |     | Γ 2 | 2          |         |       | Γ 1      | 1 2         | 2         |     |
|                                                 |        |     |            |     |     |            | =       | (𝑓 (𝑓 | (𝛾)),𝜑∘𝑔 |             | ∘𝑔 )      |     |
|                                                 |        |     |            |     |     |            |         | 1     | 2        | 2           | 1         |     |
|                                                 |        |     |            |     |     |            | =       | track | (𝑓 ∘𝑓    | , 𝑔         | ∘𝑔 )(𝛾,𝜑) | □   |
|                                                 |        |     |            |     |     |            |         |       | Γ 1      | 2 2         | 1         |     |
...
after a sequence of effects track(𝑓 ,𝑔 ),⋯,track(𝑓 ,𝑔 ) has been applied to 𝜕Γ:
|     |     |     |     | 1     | 1   |       | 𝑛 𝑛 |     |       |     |     |     |
| --- | --- | --- | --- | ----- | --- | ----- | --- | --- | ----- | --- | --- | --- |
|     |     |     |     | 𝑓     |     |       |     | 𝑓   |       |     |     |     |
|     |     |     |     | 1     |     |       |     |     | 𝑛     |     |     |     |
...
|     |     | recover |     | (track | (𝑓,𝑔)(𝛾,𝜑)) |     | =   | recover | (𝛾,𝜑) |     |     | (10) |
| --- | --- | ------- | --- | ------ | ----------- | --- | --- | ------- | ----- | --- | --- | ---- |
|     |     |         |     | Γ      | Γ           |     |     |         | Γ     |     |     |      |
Proof.
|     | recover |     | (track | (𝑓,𝑔)(𝛾,𝜑)) |     | =   | recover        | (𝑓(𝛾),𝜑∘𝑔) |     |     |     |     |
| --- | ------- | --- | ------ | ----------- | --- | --- | -------------- | ---------- | --- | --- | --- | --- |
|     |         |     | Γ      | Γ           |     |     |                | Γ          |     |     |     |     |
|     |         |     |        |             |     | =   | (𝜑(𝑔(𝑓(𝛾))),id |            | )   |     |     |     |
Γ
...
| --- | --- | --- | --- | --- | --- | --- | -------- | --- | --------- | ----- | --- | --- |
|     |     |     |     |     |     |     |          | Γ   |           | Γ     |     |     |
A sequence of pairs needs no separate argument. Let (𝑓 ,𝑔 ),⋯,(𝑓 ,𝑔 ) be applied in order
|     |     |     |     |     |     |     |     | 1   | 1   | 𝑛 𝑛 |     |     |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
from (𝛾,𝜑), and write 𝛿 = 𝛾 and 𝛿 = 𝑓 (𝛿 ). By Theorem 5 the composite track (𝑓 ,𝑔 )∘⋯∘
|     |     | 0   |     | 𝑖   | 𝑖 𝑖−1 |     |     |     |     |     | Γ 𝑛 | 𝑛   |
| --- | --- | --- | --- | --- | ----- | --- | --- | --- | --- | --- | --- | --- |
track (𝑓 ,𝑔 ) is track  of the twisted composite (𝑓 ∘⋯∘𝑓 , 𝑔 ∘⋯∘𝑔 ), and if 𝑔 (𝛿 ) = 𝛿  for
| Γ 1 1 |     | Γ   |     |     |     |     | 𝑛   | 1   | 1   | 𝑛   | 𝑖 𝑖 | 𝑖−1 |
| ----- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
every 𝑖 then (𝑔 ∘⋯∘𝑔 )(𝛿 ) = 𝛿 = 𝛾. That pair therefore meets the hypothesis of Theorem 7
|     | 1   | 𝑛   | 𝑛   | 0   |     |     |     |     |     |     |     |     |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
at 𝛾, and one application of the theorem gives
recover ((track (𝑓 ,𝑔 )∘⋯∘track (𝑓 ,𝑔 ))(𝛾,𝜑)) = recover (𝛾,𝜑) (11)
|     |     | Γ   | Γ   | 𝑛 𝑛 |     | Γ   | 1   | 1   |     |     | Γ   |     |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
Taking (𝛾,𝜑) = (𝛾 ,id ), recovery carries every state reached this way back to (𝛾 ,id ). A
|               |      | 0                                     | Γ   |     |     |     |     |     |     |     | 0   | Γ   |
| ------------- | ---- | ------------------------------------- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| pair with 𝑔∘𝑓 | = id |  meets the hypothesis at every state. |     |     |     |     |     |     |     |     |     |     |
Γ
Recovery reads a state through the quantity 𝜑(𝛾), and we refer to 𝜑(𝛾) = 𝛾  as the soundness
0
invariant of a state in 𝜕Γ.
11

3.1.2. Revertible Effect Functions
The track/recover model of the previous section takes inverses as given a priori: track (𝑓,𝑔)
Γ
...
𝜕Γ;
2. On the output side, we not only transform 𝜕Γ but also return an inverse function along-
side it, so that one effect can be undone while the others are retained: 𝜕Γ → 𝜕Γ×(𝜕Γ →
𝜕Γ), i.e., 𝜕Γ → 𝜕2Γ.
...
resulting types are the effect functions 𝔈 and their witnessed refinement 𝔈∗:
Γ Γ
Definition 8. Define the effect function 𝔈 and witnessed effect function 𝔈∗ as:
Γ Γ
𝔈 ≔ Γ → Γ×(Γ → Γ)
Γ
𝔈∗ ≔ (𝑒 : Γ → Γ×(Γ → Γ)) (12)
...
An element of 𝔈∗ chooses its inverse per state, and the constraint 𝑔(𝛿) = 𝛾 holds that choice
Γ
to reverting the effect where it was applied, leaving 𝑔 unconstrained everywhere else. A single
𝑔 with 𝑔∘𝑓 = id meets the constraint at every state at once, and induces an element of 𝔈∗ by
Γ Γ
...
1
𝜕Γ
Since effect functions 𝔈 are no longer endomorphisms on the context, they cannot be
Γ
directly composed. We therefore define a new operation for effect composition:
Definition 9. Given functions 𝑓,𝑔 ∈ 𝔈 , define their effect composition 𝑓 ⋄𝑔 as:
Γ
12
...
|     |     |     |     |     |     |      |     | 𝐥𝐞𝐭 | (𝛿,𝑠) | = 𝑔(𝛾) | 𝐢𝐧  |     |     |
(13)
|     |     |     |     |     | 𝑓   | ⋄𝑔 = | 𝛾   | ↦ 𝐥𝐞𝐭 | (𝜀,𝑡) | = 𝑓(𝛿) | 𝐢𝐧  |     |     |
| --- | --- | --- | --- | --- | --- | ---- | --- | ----- | ----- | ------ | --- | --- | --- |
(𝜀,𝑠∘𝑡)
Theorem 10. Effect composition carries the monoid structure of 𝔗  over to 𝔈 . That is,
|     |       |                             |     |     |     |     |     |           |     |     | Γ   | Γ   |     |
| --- | ----- | --------------------------- | --- | --- | --- | --- | --- | --------- | --- | --- | --- | --- | --- |
|     | 1. (𝔈 | ,⋄) is a monoid with unit 𝜂 |     |     |     |     | ≔   | 𝛾 ↦ (𝛾,id | );  |     |     |     |     |
|     |       | Γ                           |     |     |     |     | Γ   |           | Γ   |     |     |     |     |
2. the assignment (𝑓,𝑔) ↦ 𝛾 ↦ (𝑓(𝛾),𝑔) is a monoid homomorphism from 𝔗  into 𝔈 .
Γ Γ
Proof.
1. Associativity and the unit laws follow componentwise from those of ∘.
2. Write 𝑒 = 𝛾 ↦ (𝑓 (𝛾),𝑔 ); then (𝑒 ⋄𝑒 )(𝛾) = (𝑓 (𝑓 (𝛾)), 𝑔 ∘𝑔 ), which is the image of
|     |     | 𝑖       |     | 𝑖          | 𝑖   |     | 1           | 2   |     | 1 2 | 2   | 1   |     |
| --- | --- | ------- | --- | ---------- | --- | --- | ----------- | --- | --- | --- | --- | --- | --- |
|     | (𝑓  | ,𝑔 )∘(𝑓 | ,𝑔  | ), and (id |     | ,id | ) maps to 𝜂 |     | .   |     |     |     | □   |
|     |     | 1 1     | 2   | 2          | Γ   | Γ   |             |     | Γ   |     |     |     |     |
Theorem 11. Witnessing survives effect composition, and a uniform inverse witnesses at every
state. That is,
𝔈∗ is a submonoid of 𝔈
|     | 1.  |     |     |     |     | ;   |     |     |     |     |     |     |     |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
|     | Γ   |     |     |     | Γ   |     |     |     |     |     |     |     |     |
2. the homomorphism of Theorem 10 carries every pair with 𝑔∘𝑓 = id  into 𝔈∗.
|     |     |     |     |     |     |     |     |     |     |     |     | Γ Γ |     |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
Proof.
1. The unit lies in 𝔈∗ since id (𝛾) = 𝛾. For closure, take 𝑓,𝑔 ∈ 𝔈∗ and any 𝛾 ∈ Γ, and
|     |     |     |     |     | Γ   | Γ   |     |     |     |     |     | Γ   |     |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
let (𝛿,𝑠) = 𝑔(𝛾),(𝜀,𝑡) = 𝑓(𝛿), so that (𝑓 ⋄𝑔)(𝛾) = (𝜀,𝑠∘𝑡). Then 𝑠(𝛿) = 𝛾 and 𝑡(𝜀) = 𝛿,
|     | therefore (𝑠∘𝑡)(𝜀) |     |     |     | = 𝑠(𝛿) | = 𝛾. |     |     |     |     |     |     |     |
| --- | ------------------ | --- | --- | --- | ------ | ---- | --- | --- | --- | --- | --- | --- | --- |
𝑔∘𝑓 = id  gives 𝑔(𝑓(𝛾)) = 𝛾 at every 𝛾, so the image of such a pair is witnessed at every
2.
Γ
|     | state. |     |     |     |     |     |     |     |     |     |     |     | □   |
| --- | ------ | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
Just as track lifts a pair of transformations on Γ to 𝜕Γ, we define effect to lift 𝔈  to 𝔈 :
Γ 𝜕Γ
...
|     |     |     |        |     |     |     |       |     | 𝐥𝐞𝐭 (𝛿,𝑔) | = 𝑒(𝛾) | 𝐢𝐧    |      | (14) |
|     |     |     | effect |     | = 𝑒 | ↦   | (𝛾,𝜑) | ↦   |           |        |       |      |      |
|     |     |     |        | Γ   |     |     |       |     | ((𝛿,𝜑∘𝑔), | track  | (𝑔,pr | ∘𝑒)) |      |
|     |     |     |        |     |     |     |       |     |           |        | Γ     | 1    |      |
Since effect (𝑒) is itself 𝔈 , what it returns is an inverse in the sense of Definition 8 read
|     |     |     | Γ   |     | 𝜕Γ  |     |     |     |     |     |     |     |     |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
which is what pr ∘𝑒 does. The inverse therefore composes onto the accumulator it is handed,
1
exactly as track prescribes.
We can now prove properties for effect analogous to those of track.
| Theorem 13. effect preserves the ⋄ operation. That is, ∀𝑓,𝑔 |     |     |     |     |     |     |     |     |     | ∈   | 𝔈 : |     |     |
| ----------------------------------------------------------- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
Γ
|     |     |     |     |     | effect | (𝑓)⋄effect |     | (𝑔) | = effect | (𝑓  | ⋄𝑔) |     | (15) |
| --- | --- | --- | --- | --- | ------ | ---------- | --- | --- | -------- | --- | --- | --- | ---- |
|     |     |     |     |     |        | Γ          |     | Γ   |          | Γ   |     |     |      |
Proof. Take any (𝛾,𝜑) ∈ 𝜕Γ, and let (𝛿,𝑠) = 𝑔(𝛾) and (𝜀,𝑡) = 𝑓(𝛿), so that (𝑓 ⋄𝑔)(𝛾) = (𝜀,𝑠∘𝑡)
| and pr | ∘(𝑓 | ⋄𝑔) | = (pr | ∘𝑓)∘(pr |     | ∘𝑔). Then |     |     |     |     |     |     |     |
| ------ | --- | --- | ----- | ------- | --- | --------- | --- | --- | --- | --- | --- | --- | --- |
|        | 1   |     |       | 1       |     | 1         |     |     |     |     |     |     |     |
13

(effect (𝑓)⋄effect (𝑔))(𝛾,𝜑) = ((𝜀,𝜑∘𝑠∘𝑡), track (𝑠,pr ∘𝑔)∘track (𝑡,pr ∘𝑓))
|     |     | Γ   |     | Γ   |     |             |             |       | Γ 1               | Γ 1   |     |
| --- | --- | --- | --- | --- | --- | ----------- | ----------- | ----- | ----------------- | ----- | --- |
|     |     |     |     |     | =   | ((𝜀,𝜑∘𝑠∘𝑡), |             | track | (𝑠∘𝑡, (pr ∘𝑓)∘(pr | ∘𝑔))) |     |
|     |     |     |     |     |     |             |             |       | Γ 1               | 1     |     |
|     |     |     |     |     | =   | effect      | (𝑓 ⋄𝑔)(𝛾,𝜑) |       |                   |       |     |
Γ
...
1
𝜕2Γ
Between the levels, the projection pr  relates each lifted map to the map it lifts, as it does
1
for track
Γ  in Theorem 4.
Theorem 14. Let 𝑒 ∈ 𝔈 , write 𝑓 ≔ pr ∘𝑒, and let 𝑒′ ≔ effect (𝑒) with forward map 𝑓′ ≔ pr ∘
|     |     |     | Γ   |     | 1   |     |     |     | Γ   |     | 1   |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
𝑒′. Then
∘𝑓′
...
| --- | ----- | --- | ----- | --- | --- | --- | --- | --- | --- | --- | --- |
|     | 1     |     | 1     |     |     |     |     |     |     |     |     |
for each (𝛾,𝜑) ∈ 𝜕Γ, the lifted inverse 𝑔′ ≔ pr (𝑒′(𝛾,𝜑)) and the inverse 𝑔 ≔ pr (𝑒(𝛾))
|     | 2.                         |     |     |     |       |      |     | 2   |     |     | 2   |
| --- | -------------------------- | --- | --- | --- | ----- | ---- | --- | --- | --- | --- | --- |
|     | witnessed there satisfy pr |     |     |     | ∘𝑔′ = | 𝑔∘pr | .   |     |     |     |     |
|     |                            |     |     |     | 1     |      | 1   |     |     |     |     |
Proof.
1. By Definition 12, 𝑓′(𝛾,𝜑) = (𝑓(𝛾),𝜑∘𝑔), whose state is 𝑓(𝛾) = (𝑓 ∘pr )(𝛾,𝜑).
1
|     | 2. This is Theorem 4 applied to 𝑔′ |     |     |     | =   | track | (𝑔,𝑓). |     |     |     | □   |
| --- | ---------------------------------- | --- | --- | --- | --- | ----- | ------ | --- | --- | --- | --- |
Γ
Whether the lower triangle closes is settled by computing what the lifted inverse returns:
Theorem 15. Let 𝑒 ∈ 𝔈∗ and write 𝑓 ≔ pr ∘𝑒. Fix (𝛾,𝜑) ∈ 𝜕Γ, let (𝛿,𝑔) = 𝑒(𝛾), and write (Δ,𝑔′)
|                         |     |     | Γ                  |     |     | 1   |     |     |     |     |     |
| ----------------------- | --- | --- | ------------------ | --- | --- | --- | --- | --- | --- | --- | --- |
| for the value of effect |     |     | (𝑒) at (𝛾,𝜑). Then |     |     |     |     |     |     |     |     |
Γ
|     |     |     |     |     | 𝑔′(Δ) | =   | (𝛾, 𝜑∘𝑔∘𝑓) |     |     |     | (16) |
| --- | --- | --- | --- | --- | ----- | --- | ---------- | --- | --- | --- | ---- |
The state is recovered exactly. The accumulator is restored as well, equivalently effect (𝑒) ∈
Γ
𝔈∗
, if and only if 𝑔∘𝑓 = id ; and in every case (𝜑∘𝑔∘𝑓)(𝛾) = 𝜑(𝛾), so the soundness invariant
| 𝜕Γ  |     |     |     | Γ   |     |     |     |     |     |     |     |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
is preserved.
| Proof. By Definition 12, Δ |     |     |     | = (𝛿,𝜑∘𝑔) and 𝑔′ |     | =   | track | (𝑔,𝑓), so |     |     |     |
| -------------------------- | --- | --- | --- | ---------------- | --- | --- | ----- | --------- | --- | --- | --- |
Γ
...
𝛾. Membership in 𝔈∗
using 𝑔(𝛿) =  requires this to equal (𝛾,𝜑) at every input; taking 𝜑 = id
|                                             |     |     | 𝜕Γ  |     |     | Γ   |
| ------------------------------------------- | --- | --- | --- | --- | --- | --- |
| turns the equality of accumulators into 𝑔∘𝑓 |     |     | =   | id  |     |     |
, and that condition conversely gives the
Γ
...
The lower triangle therefore closes only when the inverse witnessed at 𝛾 reverts 𝑓 at
 does not carry 𝔈∗ into 𝔈∗
every state, so effect . What holds in every case is agreement
|     | Γ   |     | Γ   | 𝜕Γ  |     |     |
| --- | --- | --- | --- | --- | --- | --- |
at 𝛾: recover (𝑔′(Δ)) = recover (𝛾,𝜑), which is the whole of what Theorem 7 assumes of an
|     | Γ   |     | Γ   |     |     |     |
| --- | --- | --- | --- | --- | --- | --- |
...
𝔈∗ be applied in order from (𝛾
| Theorem 16. Let 𝑒 | ,⋯,𝑒 | ∈   |     |     | ,id ) and reverted in the reverse  |     |
| ----------------- | ---- | --- | --- | --- | ---------------------------------- | --- |
|                   | 1    | 𝑛 Γ |     |     | 0 Γ                                |     |
order. Then
...
at once.
Definition 17. For an effect function 𝑒 ∈ 𝔈 , the transformation monoid 𝔐(𝑒) is the submonoid of
Γ
...
| --- | --- | ---- | -------------- | ------ | --------- | ---- |
|     |     |      | 1              | 2      |           |      |
An effect induced by a pair (𝑓,𝑔) ∈ 𝔗  has 𝑓 and 𝑔 for its generators, the inverse it yields being
Γ
𝑔 at every state.
Lemma 18. Commutation is settled on the generators, and ⋄ enlarges no transformation
monoid. That is,
1. if every generator of 𝔐(𝑒 ) commutes with every generator of 𝔐(𝑒 ), then every element
1 2
| of 𝔐(𝑒 | ) commutes with every element of 𝔐(𝑒 |       |     | );  |     |     |
| ------ | ------------------------------------ | ----- | --- | --- | --- | --- |
|        | 1                                    |       |     | 2   |     |     |
| 2. 𝔐(𝑒 | ⋄𝑒 ) ⊆ ⟨𝔐(𝑒                          | )∪𝔐(𝑒 | )⟩. |     |     |     |
| 1      | 2                                    | 1     | 2   |     |     |     |
...
Proof.
1. The maps commuting with every generator of 𝔐(𝑒 ) form a submonoid of Γ → Γ, since
2
id lies in it and 𝑓 ∘𝑓′ does where 𝑓 and 𝑓′ do. That submonoid contains the generators of
Γ
𝔐(𝑒 ) by hypothesis and hence contains 𝔐(𝑒 ). Fixing 𝑓 ∈ 𝔐(𝑒 ), the maps commuting
1 1 1
with 𝑓 likewise form a submonoid containing the generators of 𝔐(𝑒 ) and hence 𝔐(𝑒 ).
2 2
2. By Definition 9 the forward map of 𝑒 ⋄𝑒 is (pr ∘𝑒 )∘(pr ∘𝑒 ) and the inverse it yields
1 2 1 1 1 2
at any state is 𝑠∘𝑡 for an 𝑠 yielded by 𝑒 and a 𝑡 yielded by 𝑒 . Every generator of 𝔐(𝑒 ⋄
2 1 1
𝑒 ) is therefore a composite of generators of the two. □
2
Definition 19. Effect functions 𝑒 ,𝑒 ∈ 𝔈 are independent when
1 2 Γ
...
2 2 1 2 1
and the same with 𝑒 and 𝑒 exchanged.
1 2
A family (𝑒 ) is pairwise independent when 𝑒 and 𝑒 are independent for every 𝑙 ≠ 𝑙′. A
𝑙 𝑙∈𝐿 𝑙 𝑙′
family may repeat an effect function, and holding one independent of itself is holding 𝔐(𝑒)
commutative.
For effects induced by pairs (𝑓 ,𝑔 ) and (𝑓 ,𝑔 ), clause (1) is by Lemma 18(1) the commu-
1 1 2 2
tation of the four pairs 𝑓 ,𝑓 ; 𝑔 ,𝑔 ; 𝑓 ,𝑔 ; and 𝑔 ,𝑓 , and clause (2) holds outright, an induced
1 2 1 2 1 2 1 2
effect yielding one inverse at every state. Commutation under ⋄ is a different property. What
𝑒 ⋄𝑒 = 𝑒 ⋄𝑒 equates is the composite forward map of the two orders with each other and
1 2 2 1
...
Theorem 20. Let 𝑒 ,⋯,𝑒 ∈ 𝔈∗ be pairwise independent and applied in order from 𝛾 . Write
1 𝑛 Γ 0
𝑓 ≔ pr ∘𝑒 , let 𝛿 ≔ 𝑓 (𝛿 ) with 𝛿 ≔ 𝛾 , and let 𝑔 ≔ pr (𝑒 (𝛿 )) be the inverse 𝑒 yields
𝑖 1 𝑖 𝑖 𝑖 𝑖−1 0 0 𝑖 2 𝑖 𝑖−1 𝑖
where it is applied. Fix 𝑗 and write 𝛿′ ≔ (𝑓 ∘⋯∘𝑓 )(𝛿 ) for the states of the sequence with
𝑖 𝑖 𝑗+1 𝑗−1
𝑒 omitted, so that 𝛿′ = 𝛿 . Then for every 𝑢 with 𝑗 ≤ 𝑢 ≤ 𝑛,
𝑗 𝑗 𝑗−1
1. 𝛿 = 𝑓 (𝛿′) and 𝑔 (𝛿 ) = 𝛿′;
𝑢 𝑗 𝑢 𝑗 𝑢 𝑢
2. each 𝑒 with 𝑖 > 𝑗 yields at 𝛿′ the same inverse 𝑔 it yields at 𝛿 .
𝑖 𝑖−1 𝑖 𝑖−1
Proof.
1. The first equation is an induction on 𝑢. At 𝑢 = 𝑗 it reads 𝛿 = 𝑓 (𝛿 ), which is the
𝑗 𝑗 𝑗−1
definition of 𝛿 . For the inductive step, 𝛿 = 𝑓 (𝛿 ) = 𝑓 (𝑓 (𝛿′)) = 𝑓 (𝑓 (𝛿′)) =
𝑗 𝑢+1 𝑢+1 𝑢 𝑢+1 𝑗 𝑢 𝑗 𝑢+1 𝑢
𝑓 (𝛿′ ), the middle equality being clause (1) of Definition 19 for 𝑒 and 𝑒 , which are
𝑗 𝑢+1 𝑢+1 𝑗
distinct effects of the family since 𝑢+1 > 𝑗. For the second equation, clause (1) carries
𝑔 out through the forward maps applied after 𝑒 , leaving the witness of 𝑒 to be used at
𝑗 𝑗 𝑗
the one state it holds at:
𝑔 (𝛿 ) = (𝑔 ∘𝑓 ∘⋯∘𝑓 )(𝛿 ) = (𝑓 ∘⋯∘𝑓 )(𝑔 (𝑓 (𝛿 ))) = 𝛿′
𝑗 𝑢 𝑗 𝑢 𝑗+1 𝑗 𝑢 𝑗+1 𝑗 𝑗 𝑗−1 𝑢
the last equality resting on 𝑔 (𝑓 (𝛿 )) = 𝛿 , which is the witness Definition 8 requires
𝑗 𝑗 𝑗−1 𝑗−1
of 𝑒 at 𝛿 .
𝑗 𝑗−1
16

2. By (1) the state 𝛿 is 𝑓 (𝛿′ ), and 𝑓 ∈ 𝔐(𝑒 ), so clause (2) of Definition 19 for 𝑒 and
𝑖−1 𝑗 𝑖−1 𝑗 𝑗 𝑖
𝑒 gives pr (𝑒 (𝑓 (𝛿′ ))) = pr (𝑒 (𝛿′ )). □
𝑗 2 𝑖 𝑗 𝑖−1 2 𝑖 𝑖−1
...
Corollary 21. Let 𝑒 ,⋯,𝑒 ∈ 𝔈∗ be pairwise independent and applied in order from 𝛾 , and let
1 𝑛 Γ 0
𝑔 ,⋯,𝑔 be as above. Applying the 𝑛 inverses at 𝛿 in the order of any permutation of {1,⋯,𝑛}
1 𝑛 𝑛
reaches 𝛾 .
0
Proof. By downward induction on 𝑛. Let the permutation begin with 𝑗. By Theorem 20(1) ap-
plying 𝑔 at 𝛿 reaches 𝛿′, the state the sequence with 𝑒 omitted reaches, and by Theorem 20(2)
𝑗 𝑛 𝑛 𝑗
the inverses the remaining effects yielded there are the 𝑔 in hand. That sequence is pairwise
𝑖
independent, being a subfamily, so the induction hypothesis applies to it and to the rest of the
permutation; the empty sequence reaches 𝛾 . □
0
...
Together, these constructions constitute revertible effects: each effect function in 𝔈∗ explicitly
Γ
...
Σ ≔ (𝑘 : 𝐾) ⇀ 𝒱︀ (20)
𝑘
where 𝜎 : Σ is a finite partial function assigning to each 𝑘 ∈ dom(𝜎) ⊆ 𝐾 a value of type 𝒱︀ .
𝑘
We write:
...
value type 𝒱︀ , providing static type safety for dependency access. Extension and restriction
𝑘
...
describes the transitions that do occur, applies to these operations unchanged. A reader prefer-
ring to internalize the failure may read every Σ ⇀ Σ below as Σ → 𝖬𝖺𝗒𝖻𝖾(Σ) and compose in
...
Notably, set(𝑘,𝑣) has type 𝔈∗, precisely an effect function on the coeffect context. We can
Σ
therefore directly apply the effect machinery from Section 3.1: effect provides automatic
Σ
tracking and recovery of dependency registrations. This is the synergy between reactive coef-
fects and revertible effects: coeffect operations are effects, and effects are revertible.
...
Definition 24. A coeffect at a key 𝑘 is a triple (𝒱︀ ,≃,𝒜︀ ), where 𝒱︀ is the value type of Defin-
𝑘 𝑘 𝑘
𝑘
ition 22, ≃ is an equivalence relation on 𝒱︀ up to which values at 𝑘 are compared (Section 3.3.2),
𝑘
𝑘
and 𝒜︀ is a set of coeffect operations, the operations the value bound at 𝑘 provides to a component
𝑘
18

holding it. An operation 𝑎 ∈ 𝒜︀ carries an argument type 𝑋 and an outcome type 𝐵 , and acts
𝑘 𝑎 𝑎
on the value alone:
𝑎 : 𝑋 → 𝒱︀ ⇀ 𝒱︀ ×(𝒱︀ ⇀ 𝒱︀ )×𝐵 (22)
𝑎 𝑘 𝑘 𝑘 𝑘 𝑎
its first two constituents forming an effect function on 𝒱︀ witnessed as Definition 8 requires,
𝑘
...
𝑘
≃-related values to ≃-related values, and equal outcomes. An operation acts on the coeffect
𝑘 𝑘
...
Typing an operation of 𝑘 on 𝒱︀ is what confines it to the binding at 𝑘: the lift reads and
𝑘
...
turns on another key reads that key’s value into its argument 𝑋 , and the reactive discipline of
𝑎
...
activate only once all the dependencies it declares are present, rather than accessing them opti-
mistically and failing when one is missing. This raises two questions: whether a component’s
...
transforms 𝜎 to 𝜎′ can be classified by a specification 𝑑 ∈ 𝔇 according to whether 𝑑’s satis-
Σ
faction status is altered:
Definition 26. Given a coeffect specification 𝑑 ⊆ 𝐾 and states 𝜎,𝜎′ ∈ Σ, define:
19

(cid:158)activating if 𝜎 ⊭ 𝑑∧𝜎′ ⊧ 𝑑
(cid:156)
notify (𝜎,𝜎′) ≔ deactivating if 𝜎 ⊧ 𝑑∧𝜎′ ⊭ 𝑑 (26)
𝑑 (cid:157)
(cid:156)neutral otherwise
{
...
that specification, so a loss of satisfaction is detected where it happens and drives a deactiva-
tion. Both halves are immediate from the definitions above, satisfaction being a precondition
checked where the component would activate and notify being defined at every transition.
𝑑
The criterion covers one direction of the coeffect ordering and not the other. If component
𝐴 provides a key 𝑘 and component 𝐵 declares 𝑘 ∈ 𝑑 , then 𝐵 can activate only after 𝐴 has
𝐵
activated and provided 𝑘, since 𝜎 ⊧ 𝑑 requires 𝑘 ∈ dom(𝜎). The converse fails: unloading 𝐴
𝐵
...
Coeffect Isolation. By introducing isolation realms, coeffect isolation allows the same depen-
dency to bind to different values in different contexts. This has broad applications in multi-
...
• 𝜎 : (𝑟 : 𝑅) ⇀ 𝒱︀ is the dependency table, a partial dependent function from realm identi-
𝑟
fiers to typed values.
...
𝜌(𝑘)
get = 𝑘 ↦ (𝜌,𝜎) ↦ 𝜎(𝜌(𝑘))
set : (𝑘 : 𝐾)×𝒱︀ → Σiso ⇀ Σiso ×(Σiso ⇀ Σiso)
𝜌(𝑘) (28)
...
function (𝔈∗ ) and thus inherits revertibility, whereas isolate needs none, deriving a context
Σiso
...
empty (𝜖 ) by default; and 𝜎 maps each key 𝑘 to a provider function from metadata ℳ︀  to value
| 𝑘   |     |     |     |     |     |     |     |     |     | 𝑘   |     |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
𝒱︀ . A specification 𝑑 ∈ 𝔇inter carries the component-declared metadata, assigning each key its
𝑘
metadata 𝑑(𝑘), with dom(𝑑) serving as the dependency set. Each key equips its metadata with
a monoid (ℳ︀ ,⊕ ,𝜖 ): the merge ⊕  is associative with identity 𝜖  (the empty metadata).
|     | 𝑘   | 𝑘 𝑘 |     | 𝑘   |     |     |     |     | 𝑘   |     |     |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
Definition 31. The get, set, and intercept operations on Σinter are:
|     | get | :   | (𝑘:𝐾)×ℳ︀ |     | → Σinter | ⇀   |     |         | 𝒱︀  |       |     |
| --- | --- | --- | -------- | --- | -------- | --- | --- | ------- | --- | ----- | --- |
|     |     |     |          | 𝑘   |          |     |     |         | 𝑘   |       |     |
|     | get | =   | (𝑘,𝜇)    |     | ↦ (𝜄,𝜎)  | ↦   |     | 𝜎(𝑘)(𝜇⊕ |     | 𝜄(𝑘)) |     |
𝑘
|           | set | : (𝑘:𝐾)×(ℳ︀ |          | →𝒱︀ | ) → Σinter | ⇀   |                                 | Σinter×(Σinter |        | ⇀Σinter) |      |
| --------- | --- | ----------- | -------- | --- | ---------- | --- | ------------------------------- | -------------- | ------ | -------- | ---- |
|           |     |             |          | 𝑘   | 𝑘          |     |                                 |                |        |          | (30) |
...
When a component with specification 𝑑 accesses key 𝑘, the system evaluates 𝜎(𝑘)(𝑑(𝑘)⊕
𝑘
...
section gives that unification a concrete construction, assembles from the coeffects an observa-
tional equivalence that supplies the effect independence Section 3.1.3 leaves open, and argues
...
Under this definition, effect maps 𝔈 to itself, unifying the 𝜕-tower into a single self-
Γ
∞
...
that maintains modularity while enabling unified cross-level management. The effect transfor-
mation realizes a literal “plug-in” metaphor:
...
• Components at different levels of the hierarchy are independently loadable and unload-
able; a parent context aggregates and manages the effects of all its children, enabling
...
𝜎 ≃ 𝜎′ ≔ dom(𝜎) = dom(𝜎′)∧∀𝑘 ∈ dom(𝜎). 𝜎(𝑘) ≃ 𝜎′(𝑘)
𝑘
(32)
𝛾 ≃ 𝛾′ ≔ 𝜎 ≃ 𝜎
𝛾 𝛾′
writing 𝜎 for the coeffect projection of 𝛾 (Definition 32).
𝛾
...
satisfaction predicate 𝜎 ⊧ 𝑑 and on the classification notify of Definition 26, and reactivity is a
𝑑
property of Σ/ ≃.
Calling the relation observational is a claim about each ≃, namely that it separates no more
𝑘
...
𝑥 : 𝑋 . A test over 𝒜︀ is a finite word over the generators of the monoids 𝔐(𝑎), 𝑎 ∈ 𝒜︀, each letter
𝑎
...
𝑣,𝑣′ : 𝑉 are indistinguishable, written 𝑣 ≈ 𝑣′, when every test over 𝒜︀ is defined at both or at
𝒜︀
...
𝒜︀
Every admissible choice of ≃ is therefore contained in ≈, and ≈ is itself admissible.
𝑘 𝒜︀ 𝒜︀
𝑘 𝑘
Proof.
1. Let 𝑣 ≈ 𝑣′ and let 𝑎 ∈ 𝒜︀ be applied to an argument. Prefixing a test by one letter is again
𝒜︀
...
2. Let 𝑅 be such an equivalence and 𝑣𝑅𝑣′. Each letter of a test is a forward map or a yielded
...
Two maps are related when they agree at every state, and two pairs in 𝜕Γ when both compo-
nents are:
𝑓 ≃ 𝑔 ≔ ∀𝛾 ∈ Γ. 𝑓(𝛾) ≃ 𝑔(𝛾)
(34)
(𝛿,𝑔) ≃ (𝛿′,𝑔′) ≔ 𝛿 ≃ 𝛿′ ∧𝑔 ≃ 𝑔′
...
Definition 37. Read Definition 8 up to ≃: an 𝑒 ∈ 𝔈 lies in 𝔈∗ when 𝑒 respects ≃ as a map Γ →
Γ Γ
...
Lemma 38. With 𝔈∗ read as in Definition 37, every equality of states asserted in Section 3.1 holds
Γ
with = replaced by ≃, and the accumulator of every state reachable from (𝛾 ,id ) respects ≃.
0 Γ
Proof. An accumulator is a composition of inverses, each respecting ≃ by Definition 37(2), and a
composition of maps respecting ≃ respects ≃, the base case being id . The proofs of Section 3.1
Γ
then go through unchanged, respect being what carries a relation through an inverse: from
𝑔 (𝛿 ) ≃ 𝛿 and 𝑔 (𝛿 ) ≃ 𝛾 respect gives (𝑔 ∘𝑔 )(𝛿 ) ≃ 𝛾, which is the step each composition
2 2 1 1 1 1 2 2
of inverses takes, and the soundness invariant of Theorem 7 reads 𝜑(𝛾) ≃ 𝛾 by that step. □
0
The commutation Definition 19 asks for is read up to ≃ by the same lemma, and reading it
that way is what makes it attainable at all: two operations may leave values that ≃ identifies and
𝑘
...
∀𝑥 : 𝑋 , 𝑔 ∈ 𝔐(𝑎′Σ), 𝜎 ∈ Σ. pr (𝑎Σ(𝑥)(𝑔(𝜎))) = pr (𝑎Σ(𝑥)(𝜎)) (35)
𝑎 3 3
...
key 𝑘 is commutative when any two operations of 𝒜︀ are independent, an operation being held
𝑘
...
Proof. Let 𝑎 lie in 𝒜︀ and 𝑎′ in 𝒜︀ with 𝑘 ≠ 𝑘′. By Definition 24 every generator of 𝔐(𝑎Σ) is of
𝑘 𝑘′
the form 𝜎 ↦ 𝜎[𝑘 ↦ 𝑢(𝜎(𝑘))] for a map 𝑢 on 𝒱︀ , being either the lift of a forward map or the
𝑘
...
disturbing the other. The allocator of the opening example divides by what its interface pub-
lishes. Where the handles it hands out are compared by no operation of the key, ≃ may relate
𝑘
...
outcomes compared by equality, no admissible ≃ makes the two orders of allocation agree, and
𝑘
...
speaks of.
Definition 41. The coeffect-mediated effect functions form the least set 𝔈𝒜︀ ⊆ 𝔈 that contains the
Σ Σ
unit 𝜂 and is closed under the following: for a key 𝑘, an operation 𝑎 ∈ 𝒜︀ , an argument 𝑥 :
Σ 𝑘
𝑋 , and a family (𝑒 ) of members,
𝑎 𝑏 𝑏∈𝐵
𝑎
𝜎 ↦ 𝐥𝐞𝐭 (𝛿,𝑠,𝑏) = 𝑎Σ(𝑥)(𝜎) 𝐢𝐧 𝐥𝐞𝐭 (𝜀,𝑡) = 𝑒 (𝛿) 𝐢𝐧 (𝜀, 𝑠∘𝑡) (36)
𝑏
...
Theorem 42. Let 𝑒 ,𝑒 ∈ 𝔈𝒜︀ and let every key at which operations of both occur be commu-
1 2 Σ
tative (Definition 39). Then 𝑒 and 𝑒 are independent (Definition 19).
1 2
Proof. By induction on the construction of Definition 41, 𝔐(𝑒 ) lies in the submonoid generated
𝑖
by the generators of the operations occurring in 𝑒 : the unit generates the trivial monoid, and
𝑖
...
operation occurring in 𝑒 commute with a generator of one occurring in 𝑒 . Where the two
1 2
...
For clause (2), take 𝑔 ∈ 𝔐(𝑒 ), a composite of generators of the operations occurring in 𝑒 , and
2 2
induct on the construction of 𝑒 . The unit yields id at every state. At a stage, let (𝛿,𝑠,𝑏) =
1 Σ
𝑎Σ(𝑥)(𝜎) and (𝜀,𝑡) = 𝑒 (𝛿), so that the stage yields 𝑠∘𝑡 at 𝜎. Independence of the operations,
𝑏
applied to one generator of 𝑔 at a time, yields 𝑠 and 𝑏 again at 𝑔(𝜎), so the same continuation
𝑒 is chosen, and clause (1) puts the state it runs from at 𝑔(𝛿), where the induction hypothesis
𝑏
...
order its task calls for, and Corollary 21 reverts them in whatever order the system finds conve-
nient, no two components constraining each other. The order-sensitive part is carried by the
...
Explicit state threading (functional). To preserve referential transparency, purely func-
tional languages model side effects as explicit transformations on state. The State monad 𝑆 →
...
Implicit mutation (imperative/OOP). Mainstream imperative languages permit compo-
nents to modify shared state and access dependencies without explicit declaration at the
call site. On the effect side, a representative example is React’s useEffect hook: it registers a
persistent side effect on the component’s internal fiber, yet neither the effect target nor the regis-
tration mechanism appears as an explicit parameter—identification relies on call-order position
...
automatically. For revertible effects, the developer supplies the inverse of each atomic opera-
tion, and the inverse of any composite follows by composition (Section 3.1), so a component’s
...
• 𝑑 : 𝔇 is the coeffect specification of Definition 25, declaring the dependencies required
Γ
from the environment;
• 𝑝 : 𝔓 ≔ 𝖲𝖾𝗍(𝐾) is the provision, declaring the coeffect keys the component may provide,
Γ
and no key outside 𝑝 is one its effect function writes;
• 𝑒 : 𝔈∗ is the witnessed effect function of Definition 8, defining the effects contributed
Γ
...
coeffect context being one of its projections (Definition 32), so the 𝔇 of Definition 25 is written
Σ
𝔇 here.
Γ
...
lifecycle it stands.
Definition 44. Fix a set 𝔑 of fiber names. A fiber instantiating the component (𝑑,𝑝,𝑒) ∈ ℭ is a
Γ
tuple ⟨𝑑,𝑝,𝑒,𝜋,𝜎,𝜏,𝜃⟩, where
• 𝑑 : 𝔇 , 𝑝 : 𝔓 , and 𝑒 : 𝔈∗ are the coeffect specification, provision, and effect function of
Γ Γ Γ
Definition 43;
...
the fiber;
• 𝜃 : Θ is the lifecycle state, which in the two-state model of Section 4.2 is
Γ
Θ ≔ 𝖨𝗇𝖺𝖼𝗍𝗂𝗏𝖾 | 𝖠𝖼𝗍𝗂𝗏𝖾(𝑔,𝜔) (38)
Γ
...
provided it when the transition committed. Section 4.3 replaces Θ by the extension that
Γ
...
Definition 45. Write 𝔉 for the set of fibers over Γ. A state 𝛾 ∈ Γ carries a registry
Γ
𝐹 : 𝔑 ⇀ 𝔉 (39)
𝛾 Γ
a finite partial function whose parent pointers form a tree rooted at 𝗋𝗈𝗈𝗍, together with whatever
else in Γ no fiber’s 𝜎 names. We write 𝛾(𝑛) for 𝐹 (𝑛), and abbreviate a field of 𝛾(𝑛) by
𝛾
subscripting it with 𝑛 where the state is clear, so that 𝑑 ,𝑝 ,𝑒 ,𝜋 ,𝜎 ,𝜏 ,𝜃 are the fields of
𝑛 𝑛 𝑛 𝑛 𝑛 𝑛 𝑛
Definition 44 and 𝑔 ,𝜔 the accumulator and committed view that 𝜃 carries; 𝛾[𝜃 ↦ 𝜃′], 𝛾[𝑛 ↦
𝑛 𝑛 𝑛 𝑛
...
|     | 𝜎   | ≔ ⋃{𝜎 | | 𝑚 | ∈ dom(𝐹 | ),  | 𝜃 = | 𝖠𝖼𝗍𝗂𝗏𝖾(−,−)} |     |     |     | (40) |
| --- | --- | ----- | --- | ------- | --- | --- | ------------ | --- | --- | --- | ---- |
|     | 𝛾   |       | 𝑚   |         | 𝛾   | 𝑚   |              |     |     |     |      |
The union is well defined because a fiber writes only the keys it declares, dom(𝜎 ) ⊆ 𝑝 ,
|     |     |     |     |     |     |     |     |     |     |     | 𝑛 𝑛 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
and the provisions of distinct fibers are disjoint (Definition 43), so each 𝑘 ∈ dom(𝜎 ) lies in the
𝛾
table of exactly one 𝖠𝖼𝗍𝗂𝗏𝖾 fiber, whose name we write provider (𝛾) ∈ 𝔑 and call the provider of
𝑘
𝑘. Each key therefore has one possible provider, fixed by the provisions and not by the state.
No rule writes 𝜎  directly: a fiber’s provisions are the set operations its own effect function
𝑛
performs, which land in 𝜎  and so are already part of the state 𝑒  returns, and they leave again
|     |     | 𝑛   |     |     |     |     |     | 𝑛   |     |     |     |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
𝜎 ⊧ 𝑑. A key lies in dom(𝜎 ) exactly when some 𝖠𝖼𝗍𝗂𝗏𝖾 fiber has installed it, its provision being
𝛾 𝛾
...
map 𝑑 → 𝔑, and is ⊥ when 𝑛 ought not to be running at all:
𝑛
|        |     |       | ⊥    |       |          |     | if        | 𝜏 ∨¬(𝛾 | ⊧ 𝑑 | )   |      |
| ------ | --- | ----- | ---- | ----- | -------- | --- | --------- | ------ | --- | --- | ---- |
| target |     | (𝛾) ≔ | {    |       |          |     |           | 𝑛      | 𝑛   |     | (41) |
|        | 𝑛   |       | (𝑘 ∈ | 𝑑 ) ↦ | provider | (𝛾) | otherwise |        |     |     |      |
|        |     |       |      | 𝑛     |          | 𝑘   |           |        |     |     |      |
A state is quiescent when every fiber has reached its target view:
|          |      |         |     | target | (𝛾) | = ⊥ | if  | 𝜃 = | 𝖨𝗇𝖺𝖼𝗍𝗂𝗏𝖾   |     |      |
| -------- | ---- | ------- | --- | ------ | --- | --- | --- | --- | ---------- | --- | ---- |
| quiet(𝛾) | ≔ ∀𝑛 | ∈ dom(𝐹 |     | ). {   | 𝑛   |     |     | 𝑛   |            |     | (42) |
|          |      |         | 𝛾   | target | (𝛾) | = 𝜔 | if  | 𝜃 = | 𝖠𝖼𝗍𝗂𝗏𝖾(−,𝜔 | )   |      |
|          |      |         |     |        | 𝑛   |     | 𝑛   | 𝑛   |            | 𝑛   |      |
30

The target answers to two things and to nothing else: retirement, through 𝜏 , and coeffect
𝑛
resolution, through 𝛾 ⊧ 𝑑  and provider , each declared key being read off 𝜎
 at the one shared
|     |     | 𝑛   |     |     | 𝑘   |     |     | 𝛾   |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
realm of Definition 43.
The committed view of Definition 44 has the same type as the target view, and the lifecycle
is driven by comparing them: 𝜔  is the resolution 𝑛 activated against, target (𝛾) the one it
|     |     |     |     | 𝑛   |     |     |     | 𝑛   |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
∗
whenever its premises hold. A sequence of steps interleaves the two, and ⟶ below means
lifecycle steps alone.
| 𝑛∉dom(𝐹 | )   | 𝜋 ∈dom(𝐹 | )∪{𝗋𝗈𝗈𝗍} |     | (𝑑,𝑝,𝑒)∈ℭ | ∀𝑚∈dom(𝐹 | ).𝑝∩𝑝 | =⌀  |
| ------- | --- | -------- | -------- | --- | --------- | -------- | ----- | --- |
|         | 𝛾   |          | 𝛾        |     |           | Γ        | 𝛾     | 𝑚   |
O-Insert
...
premise ∀𝑚. 𝜋 ≠ 𝑛 keeps the tree well-formed by removing children before their parent. The
𝑚
...
Definition 47. An application of 𝑒 , or one of its iterations where Section 4.3.2 applies, may
𝑛
register a component (𝑑,𝑝,𝑒) ∈ ℭ . In place of a state map it takes the O-Insert of that component
Γ
...
child, since Definition 46 does not read the fiber tree. O-Retire has 𝑛 ∈ dom(𝐹 ) as its only
𝛾
...
inventory of them.
Definition 48. A map 𝑓 : Γ → Γ is confined to 𝑛 when for every 𝛾 ∈ Γ with 𝑛 ∈ dom(𝐹 ), writing
𝛾
𝛿 = 𝑓(𝛾),
1. (Writes.) dom(𝐹 ) = dom(𝐹 ), 𝛿(𝑚) = 𝛾(𝑚) for every 𝑚 ∈ dom(𝐹 ) with 𝑚 ≠ 𝑛, and 𝛿(𝑛)
𝛿 𝛾 𝛾
and 𝛾(𝑛) differ in 𝜎 alone;
2. (Reads.) two states agreeing on 𝜎 , on the restrictions 𝜎 | for every 𝑚 ∈ dom(𝐹 ), and
𝑛 𝑚 𝑑 𝛾
𝑛
...
map pr ∘𝑒 and the inverse it yields confined to 𝑛. Every fiber’s effect function is required to
1
...
its providers, so an effect function that reads no table but 𝜎 would be unable to use its own
𝑛
coeffects. What it may not read is a table outside 𝑑 , or any control field, which is what keeps
𝑛
...
Definition 49. The lifecycle states of this section replace Θ by
Γ
Θ ≔ 𝖨𝗇𝖺𝖼𝗍𝗂𝗏𝖾(𝜁) | 𝖱𝖾𝗅𝗈𝖺𝖽𝗂𝗇𝗀(𝑖,𝑔,𝜔) | 𝖠𝖼𝗍𝗂𝗏𝖾(𝑔,𝜔) | 𝖴𝗇𝗅𝗈𝖺𝖽𝗂𝗇𝗀(𝑔,𝜔,𝜁) (43)
Γ
where 𝑖 : 𝔈iter∗ is the remaining effect iterator (Definition 51 below), 𝑔 : Γ → Γ the accumulator
Γ
...
installed (𝛾) ≔ 𝜃 ≠ 𝖨𝗇𝖺𝖼𝗍𝗂𝗏𝖾(−), failed (𝛾) ≔ ∃𝜉 ∈ Ξ. 𝜃 = 𝖨𝗇𝖺𝖼𝗍𝗂𝗏𝖾(𝜉) (44)
𝑛 𝑛 𝑛 𝑛
An installed fiber 𝑛 resolves 𝑘 to 𝑚 when 𝜔 (𝑘) = 𝑚. The quiescence of Definition 46 is read on
𝑛
the wider state space as
(cid:158)𝜁 ≠ ⊥∨target (𝛾) = ⊥ if 𝜃 = 𝖨𝗇𝖺𝖼𝗍𝗂𝗏𝖾(𝜁)
(cid:156) 𝑛 𝑛
quiet(𝛾) ≔ ∀𝑛 ∈ dom(𝐹 ). target (𝛾) = 𝜔 if 𝜃 = 𝖠𝖼𝗍𝗂𝗏𝖾(−,𝜔 ) (45)
𝛾 (cid:157) 𝑛 𝑛 𝑛 𝑛
(cid:156)⊥ otherwise
{
...
in the premise of O-Remove. Second, 𝜎 still unions the tables of 𝖠𝖼𝗍𝗂𝗏𝖾 fibers alone, so a fiber
𝛾
...
the base calculus already: an activation requires 𝛾 ⊧ 𝑑 , so a fiber declaring 𝑘 cannot activate
𝑛
...
relied (𝛾) ≔ ∃𝑚 ∈ dom(𝐹 ), 𝑘 ∈ 𝑑 . 𝑚 ≠ 𝑛∧installed (𝛾)∧𝜔 (𝑘) = 𝑛 (46)
𝑛 𝛾 𝑚 𝑚 𝑚
𝜃 =𝖠𝖼𝗍𝗂𝗏𝖾(𝑔,𝜔) target (𝛾)≠𝜔
𝑛 𝑛
L-Leave
𝛾 ⟶𝛾[𝜃 ↦𝖴𝗇𝗅𝗈𝖺𝖽𝗂𝗇𝗀(𝑔,𝜔,⊥)]
𝑛
𝜃 =𝖴𝗇𝗅𝗈𝖺𝖽𝗂𝗇𝗀(𝑔,𝜔,𝜁) ¬relied (𝛾) 𝑔(𝛾)=𝛿
𝑛 𝑛
L-Unload
𝛾 ⟶𝛿[𝜃 ↦𝖨𝗇𝖺𝖼𝗍𝗂𝗏𝖾(𝜁)]
𝑛
...
the premise ¬relied (𝛾), which we call the guard and which holds the withdrawal of 𝑘 back
𝑛
until every consumer that resolves it to 𝑛 has gone. Theorem 63 establishes both.
34

The guard is imposed per binding rather than per fiber: relied (𝛾) tests whether some
𝑛
...
𝑑 . installed (𝛾)∧𝑘 ∈ 𝑝 , a key having one possible provider there.
𝑚 𝑚 𝑛
A guard of this kind ordinarily deadlocks. What keeps it from doing so is 𝖴𝗇𝗅𝗈𝖺𝖽𝗂𝗇𝗀 together
with 𝜎 being the union over 𝖠𝖼𝗍𝗂𝗏𝖾 fibers alone: once L-Leave has marked 𝑛, its table leaves
𝛾
𝜎 , so no target view can name 𝑛 any longer, and every consumer that committed to 𝑛 is itself
𝛾
...
Definition 51. Define the effect iterator 𝔈iter and witnessed effect iterator 𝔈iter∗ as the following
Γ Γ
recursive types:
𝔈iter ≔ 𝜇ℑ. Γ → Γ×(Γ → Γ)×𝖬𝖺𝗒𝖻𝖾(ℑ)
Γ
𝔈iter∗ ≔ 𝜇ℑ. (𝑒 : Γ → Γ×(Γ → Γ)×𝖬𝖺𝗒𝖻𝖾(ℑ)) (47)
Γ
...
The witness is read at the ≃ of Definition 33, as Definition 37 reads that of 𝔈∗: an 𝑖 ∈ 𝔈iter lies in
Γ Γ
𝔈iter∗ when 𝑖 respects ≃ and each 𝑔 it yields respects ≃ and satisfies the clause above. A triple
Γ
...
The effect iterator transformation effectiter extends effect to the iterator structure through
Γ Γ
recursive invocation:
Definition 52. Define the effect iterator transformation effectiter as:
Γ
35
...
|     |     |     |     |     |     | | 𝖭𝗈𝗍𝗁𝗂𝗇𝗀  | ⇒ ((𝛿,𝜑∘𝑔),𝑡) |       |                         |     |     |
| --- | --- | --- | --- | --- | --- | ---------- | ------------- | ----- | ----------------------- | --- | --- |
|     |     |     |     |     |     | | 𝖩𝗎𝗌𝗍(𝑖′) | ⇒ 𝐥𝐞𝐭         | (𝑠,𝑟) | = effectiter(𝑖′)(𝛿,𝜑∘𝑔) |     | 𝐢𝐧  |
Γ
(𝑠,𝑡∘𝑟)
At each iteration, the inverse 𝑔 is composed onto 𝜑 in application order, so the accumulator
 naturally recovers effects in LIFO order when applied. Because effectiter lands in
𝜑∘𝑔 ∘⋯∘𝑔
| 1           | 𝑘   |               |     |     |     |     |     |     |     |     | Γ   |
| ----------- | --- | ------------- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| the same 𝜕Γ | →   | 𝜕2Γ as effect |     |     |     |     |     |     |     |     |     |
Γ  does, an iterator is an effect in its own right and can be used
...
yield operator [43], so the model maps directly onto the generators they already provide.
 of Definition 44 is read at 𝔈iter∗ from here on, and replacing the atomic
In the calculus, the 𝑒
|     |     |     | 𝑛   |     |     |     | Γ   |     |     |     |     |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
|     |     |     | 𝜃   | =𝖨𝗇𝖺𝖼𝗍𝗂𝗏𝖾(⊥) |     | 𝜔=target | (𝛾)≠⊥ |     |     |     |     |
| --- | --- | --- | --- | ------------ | --- | -------- | ----- | --- | --- | --- | --- |
|     |     |     | 𝑛   |              |     |          | 𝑛     |     |     |     |     |
L-Begin
|     |                   |     |     | 𝛾 ⟶𝛾[𝜃 | ↦𝖱𝖾𝗅𝗈𝖺𝖽𝗂𝗇𝗀(𝑒 |             | ,id | ,𝜔)]           |     |     |     |
| --- | ----------------- | --- | --- | ------ | ------------ | ----------- | --- | -------------- | --- | --- | --- |
|     |                   |     |     |        | 𝑛            |             | 𝑛 Γ |                |     |     |     |
| 𝜃   | =𝖱𝖾𝗅𝗈𝖺𝖽𝗂𝗇𝗀(𝑖,𝑔,𝜔) |     |     | target | (𝛾)≠𝜔        | (𝛿,ℎ)=(𝛾,id |     | )∨𝑖(𝛾)=(𝛿,ℎ,−) |     |     |     |
| 𝑛   |                   |     |     |        | 𝑛            |             |     | Γ              |     |     |     |
L-Divert
|     |     |     | 𝛾   | ⟶𝛿[𝜃 | ↦𝖴𝗇𝗅𝗈𝖺𝖽𝗂𝗇𝗀(𝑔∘ℎ,𝜔,⊥)] |     |     |     |     |     |     |
| --- | --- | --- | --- | ---- | -------------------- | --- | --- | --- | --- | --- | --- |
𝑛
|     | 𝜃   | =𝖱𝖾𝗅𝗈𝖺𝖽𝗂𝗇𝗀(𝑖,𝑔,𝜔) |     |     |     | target (𝛾)=𝜔 | 𝑖(𝛾)=(𝛿,ℎ,𝖩𝗎𝗌𝗍(𝑖′)) |     |     |        |     |
| --- | --- | ----------------- | --- | --- | --- | ------------ | ------------------- | --- | --- | ------ | --- |
|     |     | 𝑛                 |     |     |     | 𝑛            |                     |     |     | L-Iter |     |
↦𝖱𝖾𝗅𝗈𝖺𝖽𝗂𝗇𝗀(𝑖′,𝑔∘ℎ,𝜔)]
|     |     |     |     | 𝛾 ⟶𝛿[𝜃 |     |     |     |     |     |     |     |
| --- | --- | --- | --- | ------ | --- | --- | --- | --- | --- | --- | --- |
𝑛
|     | 𝜃   | =𝖱𝖾𝗅𝗈𝖺𝖽𝗂𝗇𝗀(𝑖,𝑔,𝜔) |     |     | target | (𝛾)=𝜔 | 𝑖(𝛾)=(𝛿,ℎ,𝖭𝗈𝗍𝗁𝗂𝗇𝗀) |     |     |     |     |
| --- | --- | ----------------- | --- | --- | ------ | ----- | ------------------ | --- | --- | --- | --- |
|     | 𝑛   |                   |     |     |        | 𝑛     |                    |     |     |     |     |
L-Finish
|     |     |     |     | 𝛾 ⟶𝛿[𝜃 |     | ↦𝖠𝖼𝗍𝗂𝗏𝖾(𝑔∘ℎ,𝜔)] |     |     |     |     |     |
...
where it is needed.
A plain effect function (𝔈 ) is the degenerate case where the first iteration already yields
Γ
𝖭𝗈𝗍𝗁𝗂𝗇𝗀. Such a transition still passes through 𝖱𝖾𝗅𝗈𝖺𝖽𝗂𝗇𝗀 and L-Divert still applies there, but the
accumulator is id  and no iteration has run, so nothing is restored and the transition installs
Γ
...
𝔈fail ≔ 𝜇ℑ. Γ → 𝖤𝗂𝗍𝗁𝖾𝗋(Ξ, Γ×(Γ → Γ)×𝖬𝖺𝗒𝖻𝖾(ℑ))
Γ
𝔈fail∗ ≔ 𝜇ℑ. (𝑒 : Γ → 𝖤𝗂𝗍𝗁𝖾𝗋(Ξ, Γ×(Γ → Γ)×𝖬𝖺𝗒𝖻𝖾(ℑ))) (49)
Γ
×((𝛾 : Γ) → (𝐥𝐞𝐭 𝖱𝗂𝗀𝗁𝗍(𝛿,𝑔,𝑜) = 𝑒(𝛾) 𝐢𝐧 𝑔(𝛿) ≃ 𝛾))
The witness constrains the 𝖱𝗂𝗀𝗁𝗍 case alone, being vacuous where the pattern does not
match, a raise having nothing to undo, and the 𝑖 that 𝖱𝖾𝗅𝗈𝖺𝖽𝗂𝗇𝗀 carries is read at 𝔈fail∗ from here
Γ
...
𝜃 =𝖱𝖾𝗅𝗈𝖺𝖽𝗂𝗇𝗀(𝑖,𝑔,𝜔) 𝑖(𝛾)=𝖫𝖾𝖿𝗍(𝜉)
𝑛
L-Raise
𝛾 ⟶𝛾[𝜃 ↦𝖴𝗇𝗅𝗈𝖺𝖽𝗂𝗇𝗀(𝑔,𝜔,𝜉)]
𝑛
...
rule at. The sequence starts at a 𝛾0 with dom(𝐹0) = ⌀, so every fiber comes into existence by
an O-Insert, whether the orchestrator’s or one an iteration takes (Definition 47). A field of 𝛾𝑡
carries the index as a superscript, so that 𝜃𝑡, 𝜔𝑡, 𝜎𝑡, 𝑔𝑡, and 𝑖𝑡 are the lifecycle state, committed
𝑛 𝑛 𝑛 𝑛 𝑛
view, table, accumulator, and remaining iterator of 𝑛 at 𝛾𝑡, and 𝐹𝑡 and 𝜎𝑡 the registry and
coeffect context of 𝛾𝑡 itself, the 𝐹 and 𝜎 of Definition 45 read there. Predicates take the state
𝛾 𝛾
as their argument and everything else as a subscript, so installed𝑡, target𝑡, relied𝑡, and quiet𝑡
𝑛 𝑛 𝑛
are the predicates of Definition 46, Definition 49, and Definition 50 at 𝛾𝑡. An episode of 𝑛 is a
maximal interval [𝑏,𝑢] of indices throughout which installed𝑡 holds. It opens at 𝑏, where 𝑏 >
𝑛
0 and ¬installed𝑏−1, the empty 𝐹0 leaving no fiber installed at the outset; it closes at 𝑢 when
𝑛
installed𝑢 and not installed𝑢+1, which a final episode need not do.
𝑛 𝑛
...
| (cid:158)pr | ∘𝑖 at L-Iter, L-Finish, and a landing L-Divert |     |     |     |
| ----------- | ---------------------------------------------- | --- | --- | --- |
| (cid:156) 1 |                                                |     |     |     |
Ψ𝑡
| ≔ 𝑔 | at L-Unload |     |     | (51) |
| --- | ----------- | --- | --- | ---- |
(cid:157)
| (cid:156)id | at every other rule |     |     |     |
| ----------- | ------------------- | --- | --- | --- |
| { Γ         |                     |     |     |     |
where 𝑖 and 𝑔 are the iterator and the accumulator that 𝜃𝑡 carries, and the edit edit𝑡 : Γ → Γ is
𝑛
...
|     | 𝛾𝑡+1 | = edit𝑡(Ψ𝑡(𝛾𝑡)) |     | (52) |
| --- | ---- | --------------- | --- | ---- |
At L-Unload, for instance, edit𝑡 is [𝜃
|     | ↦   | 𝖨𝗇𝖺𝖼𝗍𝗂𝗏𝖾(𝜁)], and at O-Remove it is the removal ∖𝑛,  |     |     |
| --- | --- | ---------------------------------------------------- | --- | --- |
𝑛
which is why the second half is an edit rather than an assignment. The fields divide along the
same seam: the tables 𝜎 , which no edit𝑡 writes once the O-Insert creating 𝑚 has set it empty,
𝑚
and the control fields 𝜃 , 𝜏 , 𝜋 , 𝑑 , 𝑝 , 𝑒  together with dom(𝐹 ), which no Ψ𝑡 writes save
| 𝑚 𝑚 | 𝑚 𝑚 𝑚 | 𝑚   | 𝛾   |     |
| --- | ----- | --- | --- | --- |
...
𝛾 ≃ 𝛿 ≔ 𝜎 ≃ 𝜎 ∧dom(𝐹 ) = dom(𝐹 )∧∀𝑛, 𝑐 ∈ {𝜃,𝜏,𝜋,𝑑,𝑝,𝑒}. 𝑐(𝛾(𝑛)) ≃ 𝑐(𝛿(𝑛))(53)
| 𝛾 𝛿 | 𝛾   | 𝛿   |     |     |
| --- | --- | --- | --- | --- |
A field of function type, as 𝑒  and the 𝑔 inside 𝜃
|     | 𝑛   | 𝑛  are, is compared as Definition 36 compares  |     |     |
| --- | --- | ---------------------------------------------- | --- | --- |
...
view, and the remaining iterator are constituents of 𝜃
𝑛 , so the third column records the writes to
them as well, and ℎ there names the inverse the iteration of the fourth column yields, id  where
Γ
...
| rule     | 𝜃𝑡               | 𝜃𝑡+1                |             | Ψ𝑡           | control fields edited |     |
| -------- | ---------------- | ------------------- | ----------- | ------------ | --------------------- | --- |
|          | 𝑛                | 𝑛                   |             |              |                       |     |
| O-Insert | undefined        | 𝖨𝗇𝖺𝖼𝗍𝗂𝗏𝖾(⊥)         |             | id           | dom(𝐹 )               |     |
|          |                  |                     |             | Γ            | 𝛾                     |     |
| O-Retire | unconstrained    | unchanged           |             | id           | 𝜏                     |     |
|          |                  |                     |             | Γ            | 𝑛                     |     |
| O-Remove | 𝖨𝗇𝖺𝖼𝗍𝗂𝗏𝖾(−)      | undefined           |             | id           | dom(𝐹 )               |     |
|          |                  |                     |             | Γ            | 𝛾                     |     |
| L-Begin  | 𝖨𝗇𝖺𝖼𝗍𝗂𝗏𝖾(⊥)      | 𝖱𝖾𝗅𝗈𝖺𝖽𝗂𝗇𝗀(𝑒         | 𝑛 ,id Γ ,𝜔) | id Γ         | 𝜃 𝑛                   |     |
|          | 𝖱𝖾𝗅𝗈𝖺𝖽𝗂𝗇𝗀(𝑖,𝑔,𝜔) | 𝖱𝖾𝗅𝗈𝖺𝖽𝗂𝗇𝗀(𝑖′,𝑔∘ℎ,𝜔) |             | pr ∘𝑖        | 𝜃                     |     |
| L-Iter   |                  |                     |             | 1            | 𝑛                     |     |
|          | 𝖱𝖾𝗅𝗈𝖺𝖽𝗂𝗇𝗀(𝑖,𝑔,𝜔) | 𝖠𝖼𝗍𝗂𝗏𝖾(𝑔∘ℎ,𝜔)       |             | pr ∘𝑖        | 𝜃                     |     |
| L-Finish |                  |                     |             | 1            | 𝑛                     |     |
|          | 𝖱𝖾𝗅𝗈𝖺𝖽𝗂𝗇𝗀(𝑖,𝑔,𝜔) | 𝖴𝗇𝗅𝗈𝖺𝖽𝗂𝗇𝗀(𝑔∘ℎ,𝜔,⊥)  |             | id  or pr ∘𝑖 | 𝜃                     |     |
| L-Divert |                  |                     |             | Γ 1          | 𝑛                     |     |
|          | 𝖱𝖾𝗅𝗈𝖺𝖽𝗂𝗇𝗀(𝑖,𝑔,𝜔) | 𝖴𝗇𝗅𝗈𝖺𝖽𝗂𝗇𝗀(𝑔,𝜔,𝜉)    |             | id           | 𝜃                     |     |
| L-Raise  |                  |                     |             | Γ            | 𝑛                     |     |
|          | 𝖠𝖼𝗍𝗂𝗏𝖾(𝑔,𝜔)      | 𝖴𝗇𝗅𝗈𝖺𝖽𝗂𝗇𝗀(𝑔,𝜔,⊥)    |             | id           | 𝜃                     |     |
| L-Leave  |                  |                     |             | Γ            | 𝑛                     |     |
|          | 𝖴𝗇𝗅𝗈𝖺𝖽𝗂𝗇𝗀(𝑔,𝜔,𝜁) | 𝖨𝗇𝖺𝖼𝗍𝗂𝗏𝖾(𝜁)         |             | 𝑔            | 𝜃                     |     |
| L-Unload |                  |                     |             |              | 𝑛                     |     |
...
present at 𝛾𝑡:
| 1. 𝜎𝑡+1 | ≠ 𝜎𝑡  only where step 𝑡 acts on 𝑚, the write lying inside Ψ𝑡; |     |     |     |     |     |
| ------- | ------------------------------------------------------------- | --- | --- | --- | --- | --- |
| 𝑚       | 𝑚                                                             |     |     |     |     |     |
2. 𝜔  comes into existence only where step𝑡 = L-Begin(𝑛) and ceases only where step𝑡 =
𝑛
L-Unload(𝑛), so 𝜔𝑡 is constant for 𝑡 in an episode of 𝑛;
𝑛
Ψ𝑡 = 𝑔𝑡 only where step𝑡 = L-Unload(𝑛), and no other step applies 𝑔
| 3.  | 𝑛   |     |     |     | 𝑛  to the state; |     |
| --- | --- | --- | --- | --- | ---------------- | --- |
4. ¬installed𝑡 ∧installed𝑡+1 ⇒ step𝑡 = L-Begin(𝑛),  and  installed𝑡 ∧¬installed𝑡+1 ⇒
|       | 𝑛              | 𝑛   |     |     | 𝑛   | 𝑛   |
| ----- | -------------- | --- | --- | --- | --- | --- |
| step𝑡 | = L-Unload(𝑛); |     |     |     |     |     |
5. 𝜋 , 𝑑 , 𝑝 , and 𝑒  come into existence with the entry of 𝑛 and are never written again,
| 𝑛     | 𝑛 𝑛 𝑛                                                    |     |     |     |     |     |
| ----- | -------------------------------------------------------- | --- | --- | --- | --- | --- |
| and 𝜏 |  is monotone, written only at ⊤ and only by an O-Retire. |     |     |     |     |     |
𝑛
Proof. Let step 𝑡 apply 𝑟 at 𝑛. By Definition 53 it factors as edit𝑡 ∘Ψ𝑡, where edit𝑡 writes the fields
the fifth column of Table 1 names and nothing else, and Ψ𝑡 is id , an application of one of 𝑛’s
Γ
iterations, or the accumulator 𝑔𝑡, which is a composite of the inverses those iterations yielded.
𝑛
Each of the three is confined to 𝑛 by Definition 48, so Ψ𝑡 writes no field of a fiber present at 𝛾𝑡
but 𝜎 , together with the entry a registration adds and the 𝜏 its inverse writes. The two halves
𝑛
...
(1) An edit𝑡 writes no table, the fifth column naming none, and a Ψ𝑡 writes no 𝜎
 for a present
𝑚
| 𝑚 ≠ 𝑛. So 𝜎 |  can move only at 𝑚 | = 𝑛, and only inside Ψ𝑡. |     |     |     |     |
| ----------- | ------------------- | ------------------------ | --- | --- | --- | --- |
𝑚
(2) 𝜔  is a constituent of 𝜃 , which only an edit𝑡 writes and only at the fiber the step acts on,
| 𝑛   |     | 𝑛   |     |     |     |     |
| --- | --- | --- | --- | --- | --- | --- |
so by the reading above 𝜔  comes into existence at an L-Begin of 𝑛 and ceases at an L-Unload
𝑛
of 𝑛. An episode of 𝑛 is an interval on which installed  holds, hence one throughout which 𝜔
|     |     |     | 𝑛   |     |     | 𝑛   |
| --- | --- | --- | --- | --- | --- | --- |
...
, and no edit𝑡 applies a map to the state at all.
| a forward map pr | ∘𝑖 or id |     |     |     |     |     |
| ---------------- | -------- | --- | --- | --- | --- | --- |
|                  | 1        | Γ   |     |     |     |     |
(4) installed  is 𝜃 ≠ 𝖨𝗇𝖺𝖼𝗍𝗂𝗏𝖾(−), and by the reading above L-Begin and L-Unload are the only
𝑛 𝑛
rules whose premise and conclusion differ in whether 𝜃  is 𝖨𝗇𝖺𝖼𝗍𝗂𝗏𝖾. A step acting on some 𝑚 ≠
𝑛
𝑛 writes no 𝜃 , and the entry a registration adds is at a name not present at 𝛾𝑡.
𝑛
40
...
relation keeps. A premise matching 𝜃 or 𝜏 against a pattern, and the premise ∀𝑚. 𝜋 ≠ 𝑛
𝑛 𝑛 𝑚
of O-Remove, read control fields. The premises (𝑑,𝑝,𝑒) ∈ ℭ and ∀𝑚. 𝑝∩𝑝 = ⌀ of O-Insert
Γ 𝑚
read 𝑑, 𝑝, and 𝑒. A premise mentioning target or relied reads 𝜏 , the committed views inside
𝑛 𝑛 𝑛
the 𝜃 , and dom(𝜎 ), which Definition 45 computes from the 𝜃 and the dom(𝜎 ), and Defin-
𝑚 𝛾 𝑚 𝑚
ition 33 relates two coeffect contexts only where their domains agree. The remaining premises
read dom(𝐹 ). None reads a value 𝜎 (𝑘) otherwise than up to ≃, so no premise separates two
𝛾 𝛾
𝑘
≃-related states.
...
≃: it is id , or an iteration of 𝑒 , which Definition 51 requires to respect ≃, or the accumulator
Γ 𝑛
inside 𝜃 , a composite of inverses each respecting ≃ by the same definition. □
𝑛
The names a state carries are read by two of those observations, dom(𝐹 ) and the indexing
𝛾
...
registry 𝐹 ∘𝜒−1, with every name occurring in a 𝜋 or an 𝜔 replaced by its image. Then 𝜒⋅
𝛾 𝑚 𝑚
...
freshness 𝑛 ∉ dom(𝐹 ) of O-Insert and the ∀𝑚. 𝜋 ≠ 𝑛 of O-Remove, or through a table of
𝛾 𝑚
names, as target and relied read the 𝜋 and the 𝜔 . A bijection preserves each such compar-
𝑛 𝑛 𝑚 𝑚
ison. The only names a rule writes are the 𝜋 that O-Insert sets and the 𝜔 that L-Begin sets, both
...
Lemma 57. (Vestigial entries.) Call 𝑛 vestigial at 𝛾 when 𝜏 = ⊤, 𝜃 = 𝖨𝗇𝖺𝖼𝗍𝗂𝗏𝖾(⊥), 𝜎 = ⌀, and
|     |     |     |     |     |     | 𝑛 𝑛 | 𝑛   |     |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
no 𝑚 has 𝜋 = 𝑛; a vestigial entry satisfies 𝛾 ≈ 𝛾 ∖𝑛. If 𝑛 is vestigial at 𝛾 then for every rule
𝑚
| and every 𝑚 | ≠ 𝑛: |     |     |     |     |     |     |     |
| ----------- | ---- | --- | --- | --- | --- | --- | --- | --- |
a rule applying at 𝛾 acting on 𝑚 applies at 𝛾 ∖𝑛 acting on 𝑚, and the states the two reach
1.
...
| drawing the name 𝑛 or claiming a key of 𝑝 |     |     |     |     | .   |     |     |     |
| ----------------------------------------- | --- | --- | --- | --- | --- | --- | --- | --- |
𝑛
Proof. A vestigial 𝑛 contributes to no observation a premise of a rule acting on 𝑚 ≠ 𝑛 reads. It
is not 𝖠𝖼𝗍𝗂𝗏𝖾, so 𝜎  enters no 𝜎  and 𝑛 is the provider of no key, leaving 𝛾 ⊧ 𝑑  and target
|     | 𝑛   | 𝛾   |     |     |     |     | 𝑚   | 𝑚   |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
unmoved; installed  fails, so 𝑛 contributes no disjunct to relied ; no 𝜋  names 𝑛, so the
|     | 𝑛   |     |     |     |     | 𝑚   | 𝑚′  |     |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
premise ∀𝑚′. 𝜋 ≠ 𝑚 of an O-Remove of 𝑚 is unmoved; and 𝜃 , 𝜏 , and 𝜋  are read by rules
|     | 𝑚′  |     |     |     |     | 𝑛 𝑛 | 𝑛   |     |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
| confined to 𝑚 by Definition 48, so it leaves 𝜎 |     |     |     |  empty. |     |     |     | □   |
| ---------------------------------------------- | --- | --- | --- | ------- | --- | --- | --- | --- |
𝑛
...
Definition 58. A registry 𝐹  is well formed when, for all 𝑚,𝑛 ∈ dom(𝐹 ) and all 𝑘 ∈ 𝐾,
|        |                 | 𝛾    |     |     |     | 𝛾   |     |     |
| ------ | --------------- | ---- | --- | --- | --- | --- | --- | --- |
| 1. 𝜋 ∈ | dom(𝐹 )∪{𝗋𝗈𝗈𝗍}; |      |     |     |     |     |     |     |
| 𝑛      | 𝛾               |      |     |     |     |     |     |     |
| 2. 𝑚 ≠ | 𝑛 ⇒ 𝑝 ∩𝑝        | = ⌀; |     |     |     |     |     |     |
𝑚 𝑛
| 3. installed | (𝛾) ⇒ 𝜔 |  is total on 𝑑 |  and valued in dom(𝐹 |             |      | );  |     |     |
| ------------ | ------- | -------------- | -------------------- | ----------- | ---- | --- | --- | --- |
|              | 𝑛       | 𝑛              | 𝑛                    |             |      | 𝛾   |     |     |
| 4. installed | (𝛾)∧𝑘   | ∈ 𝑑 ∧𝜔 (𝑘)     | = 𝑚                  | ⇒ installed | (𝛾). |     |     |     |
|              | 𝑛       | 𝑛 𝑛            |                      |             | 𝑚    |     |     |     |
...
Theorem 59. (Preservation.) If 𝐹𝑡 is well formed then so is 𝐹𝑡+1, whichever rule step 𝑡 applies.
Each clause is established at 𝛾𝑡+1 from all four at 𝛾𝑡.
Proof. Let step 𝑡 act on 𝑛.
(1) By Table 1 only O-Insert and O-Remove write a 𝜋 or dom(𝐹 ). O-Insert has 𝜋 ∈ dom(𝐹𝑡)∪
|     |     |     |     |     |     | 𝛾   | 𝑛   |     |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
{𝗋𝗈𝗈𝗍} as a premise, which is the clause for the fiber it adds, and it leaves every other 𝜋 alone
42

while enlarging dom(𝐹 ). O-Remove has ∀𝑚. 𝜋 ≠ 𝑛, so no surviving 𝜋 names the fiber it
𝛾 𝑚 𝑚
takes away.
(2) The last premise of O-Insert is ∀𝑚. 𝑝 ∩𝑝 = ⌀, which is the clause for the fiber it adds,
𝑛 𝑚
and by Table 1 no other rule writes a 𝑝 or enlarges dom(𝐹 ). Two consequences are used below:
𝛾
dom(𝜎 ) ⊆ 𝑝 by Definition 43, so distinct tables are disjoint and 𝜎 is a function; and 𝑘 ∈ 𝑝 ∩
𝑚 𝑚 𝛾 𝑚
𝑝 forces 𝑚 = 𝑚′, so 𝑘 has at most one possible provider.
𝑚′
(3) By Lemma 54(2) the only rule that writes an 𝜔 is L-Begin, whose premise 𝜔 = target𝑡 ≠ ⊥
𝑛 𝑛
makes it total on 𝑑 and valued in dom(𝐹𝑡), target naming providers. By Table 1 the only rule
𝑛
that shrinks dom(𝐹 ) is O-Remove, whose premise 𝜃𝑡 = 𝖨𝗇𝖺𝖼𝗍𝗂𝗏𝖾(−) gives ¬installed𝑡, whence
𝛾 𝑛 𝑛
by clause (4) at 𝛾𝑡 no 𝑚 has 𝜔𝑡 (𝑘) = 𝑛 for a 𝑘 ∈ 𝑑 while installed𝑡 ; and 𝑛 itself carries no 𝜔.
𝑚 𝑚 𝑚
(4) By Lemma 54(2) and (4) the clause can fail at 𝛾𝑡+1 only where some installed has fallen,
some 𝜔 has been written, or a fiber some 𝜔 names has left dom(𝐹 ). The last is an O-Remove,
𝛾
whose removed fiber is not installed and hence, by clause (4) at 𝛾𝑡, is named by no 𝜔𝑡 of an
𝑚
installed 𝑚. The first is an L-Unload of 𝑛, whose premise ¬relied𝑡 reads
𝑛
∀𝑚 ≠ 𝑛, 𝑘 ∈ 𝑑 . installed𝑡 ⇒ 𝜔𝑡 (𝑘) ≠ 𝑛
𝑚 𝑚 𝑚
and which writes no 𝜔 for 𝑚 ≠ 𝑛 and leaves ¬installed𝑡+1, so the clause holds of 𝑛 as well.
𝑚 𝑛
The second is an L-Begin of 𝑛, writing target𝑡, whose values are the providers of the keys of
𝑛
𝑑 and hence 𝖠𝖼𝗍𝗂𝗏𝖾 at 𝛾𝑡; the step alters no other fiber’s 𝜃, so they are installed at 𝛾𝑡+1 too. □
𝑛
The guard on L-Unload is what carries clauses (3) and (4). The premise ∀𝑚. 𝜋 ≠ 𝑛 of O-
𝑚
...
Local temporal composability recovers one sequence of effects with one accumulator (Sec-
tion 3.1.3). The registry holds one accumulator per fiber and the fibers interleave: between the
moment 𝑛 composes an inverse onto 𝑔 and the moment 𝑔 runs, other fibers have moved the
𝑛 𝑛
state. Whether 𝑔 still undoes what it was built to undo there is what the global form of the
𝑛
guarantee asserts, and the condition it turns on is that the intervening steps commute with 𝑔 .
𝑛
Definition 60. For 𝑖 ∈ 𝔈iter∗ let reach(𝑖) be the least set of iterators containing 𝑖 and closed under
Γ
...
∀𝑓 ∈ 𝔐(𝑖), 𝑔 ∈ 𝔐(𝑗). 𝑓 ∘𝑔 ≃ 𝑔∘𝑓
(55)
∀𝑖′ ∈ reach(𝑖), 𝑔 ∈ 𝔐(𝑗), 𝛾 ∈ Γ. pr (𝑖′(𝑔(𝛾))) ≃ pr (𝑖′(𝛾))
2,3 2,3
and symmetrically in 𝑗, reading ≃ on maps as Definition 36 does, on a continuation as Defin-
ition 51 does, and on a registering iteration (Definition 47) as agreement of the component it
names. A family (𝑖 ) of iterators is pairwise independent when 𝑖 and 𝑖 are independent for
𝑙 𝑙∈𝐿 𝑙 𝑙′
every 𝑙 ≠ 𝑙′, and a sequence of steps is pairwise independent when (𝑒 ) is, where 𝑁 is the
𝑛 𝑛∈𝑁
set of names the sequence ever holds, one for each fiber the orchestrator inserts and each fiber
an iteration registers.
Independence in this sense is what trace theory takes as primitive: commuting actions gen-
erate an equivalence on sequences under which reordering two adjacent independent actions
...
episode of 𝑛 open at 𝑏, let 𝑢 ≥ 𝑏 lie in it, and let 𝑡 < ⋯ < 𝑡 be the indices in [𝑏,𝑢) at which the
1 𝑙
acting fiber is not 𝑛. Then
𝑔𝑢(𝛾𝑢) ≈ (Ψ𝑡 𝑙 ∘⋯∘Ψ𝑡 1)(𝛾𝑏) (56)
𝑛
...
an L-Begin, the episode opening by Definition 53, so 𝑔𝑏 = id by Table 1, the index set is empty,
𝑛 Γ
and the claim is 𝛾𝑏 ≈ 𝛾𝑏. Two facts are used at each step. Since edit𝑡 writes control fields only,
𝛾𝑡+1 ≈ Ψ𝑡(𝛾𝑡)
and since every map in 𝔐(𝑒 ) writes no control field but those a registration adds, by Defin-
𝑛
ition 48 together with Definition 47, each such map carries ≈-equal states to ≈-equal states.
Let step 𝑢 act on 𝑛. Since the episode is open at 𝑢 and 𝑢+1, Lemma 54(4) excludes an L-Begin
and an L-Unload of 𝑛, and O-Insert and O-Remove read a 𝜃 that installed𝑢 denies, leaving
𝑛 𝑛
two cases. Where the rule is L-Iter, L-Finish, or a landing L-Divert, Table 1 gives Ψ𝑢 = pr ∘𝑖𝑢
1 𝑛
and 𝑔𝑢+1 = 𝑔𝑢 ∘ℎ for the inverse ℎ that iteration yields. The witness condition of Definition 51
𝑛 𝑛
reads ℎ(Ψ𝑢(𝛾𝑢)) = 𝛾𝑢, up to ≈ where the iteration registers a fiber (Lemma 57), and 𝑔𝑢 carries
𝑛
≈ by the equation above, so
44

|     |     | 𝑔𝑢+1(𝛾𝑢+1) |     | ≈ (𝑔𝑢 | ∘ℎ)(Ψ𝑢(𝛾𝑢)) |     | = 𝑔𝑢(𝛾𝑢) |     |     |     |
| --- | --- | ---------- | --- | ----- | ----------- | --- | -------- | --- | --- | --- |
|     |     | 𝑛          |     |       | 𝑛           |     | 𝑛        |     |     |     |
Where the rule is L-Leave, L-Raise, an aborting L-Divert, or an O-Retire of 𝑛, Table 1 gives
| Ψ𝑢  and 𝑔𝑢+1 | 𝑔𝑢, so the same equation holds with ℎ |     |     |     |     |     |     |                                |     |     |
| ------------ | ------------------------------------- | --- | --- | --- | --- | --- | --- | ------------------------------ | --- | --- |
| = id         | =                                     |     |     |     |     |     | =   | id . Either way the induction  |     |     |
| Γ            | 𝑛                                     | 𝑛   |     |     |     |     |     | Γ                              |     |     |
hypothesis carries over with the index set unchanged, which is the computation of Theorem 7
one step at a time.
Let step 𝑢 act on 𝑚 ≠ 𝑛. Then 𝑔𝑢+1 = 𝑔𝑢 by Table 1, and Ψ𝑢 ∈ 𝔐(𝑒 ), or Ψ𝑢 = id  where the
|     |     |     | 𝑛   | 𝑛   |     |     |     | 𝑚   | Γ   |     |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
rule is an orchestration rule, so independence gives
|                                                     |     | 𝑔𝑢(𝛾𝑢+1) |     | ≈ 𝑔𝑢(Ψ𝑢(𝛾𝑢)) |     | =   | Ψ𝑢(𝑔𝑢(𝛾𝑢)) |     |     |     |
| --------------------------------------------------- | --- | -------- | --- | ------------ | --- | --- | ---------- | --- | --- | --- |
|                                                     |     | 𝑛        |     | 𝑛            |     |     | 𝑛          |     |     |     |
...
an episode of 𝑛 open at 𝑏 and close at 𝑢, whatever outcome 𝑛 arrives at. Then, with 𝑡 < ⋯ < 𝑡
|     |     |     |     |     |     |     |     |     | 1   | 𝑙   |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
as in Theorem 61,
|     |     |     | 𝛾𝑢+1 | ≈ (Ψ𝑡 | ∘⋯∘Ψ𝑡 | 1)(𝛾𝑏) |     |     |     | (57) |
| --- | --- | --- | ---- | ----- | ----- | ------ | --- | --- | --- | ---- |
𝑙
A fiber removed by O-Remove leaves nothing behind either, its premise admitting only 𝜃 =
𝑛
𝖨𝗇𝖺𝖼𝗍𝗂𝗏𝖾(−).
Proof. By Lemma 54(4) step 𝑢 is an L-Unload of 𝑛, whose Ψ𝑢 is 𝑔𝑢 by Lemma 54(3), so 𝛾𝑢+1
≈
𝑛
𝑔𝑢(𝛾𝑢) and Theorem 61 applies. Neither the statement nor ≈ mentions 𝜁, which by Table 1 is
𝑛
...
fixity of 𝜔  over an episode that Lemma 54(2) establishes. The ordering theorem is what
𝑛
...
|     |     |     |     | step𝑡 = | L-Begin(𝑚) | ⇒ 𝛾𝑡 | ⊧ 𝑑 |     |     | (58) |
| --- | --- | --- | --- | ------- | ---------- | ---- | --- | --- | --- | ---- |
𝑚
Let further [𝑏′,𝑢′] be an episode of 𝑚 with 𝜔𝑏′(𝑘)
|     |     |     |     |     |     | = 𝑛 for some 𝑚 |     | ≠   | 𝑛 and 𝑘 ∈ 𝑑 , let [𝑏,𝑢] be  |     |
| --- | --- | --- | --- | --- | --- | -------------- | --- | --- | --------------------------- | --- |
𝑚 𝑚
...
| ---- | ----------------- | -------------------- | --- | ------- | --- | --- | --- | --- | --- | --- |
|      | dom(𝜎𝑡) and 𝜎𝑡(𝑘) |                      |     | 𝜎𝑏′(𝑘). |     |     |     |     |     |     |
| 3. 𝑘 | ∈                 |                      | =   |         |     |     |     |     |     |     |
|      |                   | 𝑛                    | 𝑛   | 𝑛       |     |     |     |     |     |     |
Proof. The first claim is the premise target𝑡 ≠ ⊥ of L-Begin, which by Definition 46 gives 𝛾𝑡 ⊧
𝑚
𝑑 .
𝑚
(1) is Lemma 54(2).
|                            |     |     | −1 writes 𝜔𝑏′ |     | target𝑏′−1, whose values are providers, so 𝜃𝑏′ |     |     |     |     |     |
| -------------------------- | --- | --- | ------------- | --- | ---------------------------------------------- | --- | --- | --- | --- | --- |
| For (2), the L-Begin at 𝑏′ |     |     |               |     | =                                              |     |     |     |     | =   |
|                            |     |     |               |     | 𝑚                                              | 𝑚   |     |     |     | 𝑛   |
𝖠𝖼𝗍𝗂𝗏𝖾(−,−); the L-Begin at 𝑏−1 leaves 𝜃𝑏 𝑏′ and hence 𝑏 𝑏′,
|     |     |     |     |     | =   | 𝖱𝖾𝗅𝗈𝖺𝖽𝗂𝗇𝗀(−,−,−), so 𝑏 |     |     | ≠   | <   |
| --- | --- | --- | --- | --- | --- | ---------------------- | --- | --- | --- | --- |
𝑛
both episodes opening by Definition 53. Let [𝑏,𝑢] close and suppose 𝑢 ≤ 𝑢′. Then 𝑢 ∈ [𝑏′,𝑢′],
so installed𝑢  and, by (1), 𝜔𝑢 (𝑘) = 𝑛; that is relied𝑢, which the L-Unload at 𝑢 denies. Hence
|     | 𝑚   |     | 𝑚   |     |     | 𝑛   |     |     |     |     |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
𝑢′ < 𝑢.
For (3), 𝑛 is the provider of 𝑘 at 𝛾𝑏′ dom(𝜎𝑏′). No L-Unload of 𝑛 falls in [𝑏′,𝑢′]: where
|     |     |     |     | , so 𝑘 | ∈   |     |     |     |     |     |
| --- | --- | --- | --- | ------ | --- | --- | --- | --- | --- | --- |
𝑛
[𝑏,𝑢] closes it falls at 𝑢 > 𝑢′ by (2), and where it does not, Lemma 54(4) leaves 𝑛 with no L-
Unload at all. Since 𝜃𝑏′
= 𝖠𝖼𝗍𝗂𝗏𝖾(−,−), Table 1 therefore leaves L-Leave as the only rule 𝑛 can
𝑛
be acted on by within [𝑏′,𝑢′], and its Ψ𝑡 is id ; by Lemma 54(1) 𝜎  is constant there. □
|     |     |     |     |     | Γ   |     |     | 𝑛   |     |     |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
A transition spread over steps could otherwise install effects computed against a resolu-
tion that has changed under it, and two premises prevent that. L-Iter and L-Finish carry
target (𝛾) = 𝜔, so a transition proceeds only while its committed view is still its target view,
𝑛
...
Theorem 64. (Resolution coherence.) Let an episode [𝑏,𝑢] of 𝑛 open at 𝑏 with 𝜔𝑏 = 𝜔. Then
𝑛
𝜃  is 𝖱𝖾𝗅𝗈𝖺𝖽𝗂𝗇𝗀(−,−,−) on an initial interval [𝑏,𝑟] of the episode, and every iteration of the
𝑛
transition runs against the one resolution 𝜔:
|     |     |             | step𝑡 |                           |     |     |     | target𝑡 |     |      |
| --- | --- | ----------- | ----- | ------------------------- | --- | --- | --- | ------- | --- | ---- |
|     |     | ∀𝑡 ∈ [𝑏,𝑟]. |       | ∈ {L-Iter(𝑛),L-Finish(𝑛)} |     |     | ⇒   |         | = 𝜔 | (59) |
𝑛
Where the fiber leaves that interval, so that 𝑟 < 𝑢, exactly one of the following holds:
| 1. step𝑟 | =   | L-Finish(𝑛) and 𝜃𝑟+1 |     | =   | 𝖠𝖼𝗍𝗂𝗏𝖾(−,𝜔); |     |     |     |     |     |
| -------- | --- | -------------------- | --- | --- | ------------ | --- | --- | --- | --- | --- |
𝑛
2. step𝑟 ∈ {L-Divert(𝑛),L-Raise(𝑛)}, and the episode closes at some 𝑢 > 𝑟 with 𝛾𝑢+1 ≈
| (Ψ𝑡 | ∘⋯∘Ψ𝑡 | 1)(𝛾𝑏) as in Corollary 62. |     |     |     |     |     |     |     |     |
| --- | ----- | -------------------------- | --- | --- | --- | --- | --- | --- | --- | --- |
𝑙
Proof. The L-Begin at 𝑏−1 writes 𝖱𝖾𝗅𝗈𝖺𝖽𝗂𝗇𝗀, and by Table 1 it is the one rule leading into that
lifecycle state; its premise 𝜃 = 𝖨𝗇𝖺𝖼𝗍𝗂𝗏𝖾(⊥) and Lemma 54(4) put any second application of it
𝑛
outside the episode. So 𝖱𝖾𝗅𝗈𝖺𝖽𝗂𝗇𝗀 occupies an initial interval [𝑏,𝑟] of [𝑏,𝑢] and is not re-entered.
46

The first claim is then the premise target (𝛾) = 𝜔′ that Table 1 gives L-Iter and L-Finish,
𝑛
together with 𝜔′ = 𝜔 by Lemma 54(2).
For the dichotomy, step𝑟 is a rule whose premise has 𝜃 = 𝖱𝖾𝗅𝗈𝖺𝖽𝗂𝗇𝗀(−,−,−) and whose
𝑛
...
creator writes is 𝜏 , through the primitive of Definition 47, and 𝜏 is monotone by Lemma 54(5).
𝑛
...
Theorem 66. (Progress.) Assume ≺ acyclic, len(𝑒 ) ≤ 𝐾 for every 𝑛, and the set 𝑁 of names of
𝑛
Definition 60 finite; and let every step apply a lifecycle rule. Write 𝑆(𝑛) for the number of steps
acting on 𝑛 and
𝑉(𝑛) ≔ |{𝑡 : target𝑡 ≠ target𝑡+1}| (61)
𝑛 𝑛
for the number of times its target view turns. Then
1. (No deadlock.) ¬quiet𝑡 implies that some lifecycle rule applies at 𝛾𝑡;
2. (Termination.) 𝑆(𝑛) ≤ (𝐾 +4)(𝑉(𝑛)+1), and both 𝑉(𝑛) and ∑ 𝑆(𝑛) are finite.
𝑛
Consequently every maximal sequence of lifecycle steps ends in a quiescent state.
Proof. No deadlock. Let ¬quiet𝑡, so some fiber 𝑛 satisfies neither clause of the quiet of Defini-
tion 49. Reading Table 1 against the four kinds it can then be:
• 𝜃𝑡 = 𝖨𝗇𝖺𝖼𝗍𝗂𝗏𝖾(⊥) with target𝑡 ≠ ⊥: L-Begin applies;
𝑛 𝑛
47

| 𝜃𝑡  |     | ) with target𝑡 |     |     |     |     |     |     |     |
| --- | --- | -------------- | --- | --- | --- | --- | --- | --- | --- |
• = 𝖱𝖾𝗅𝗈𝖺𝖽𝗂𝗇𝗀(−,−,𝜔 = 𝜔 : whichever of L-Iter, L-Finish, and L-Raise
| 𝑛   |     | 𝑛   |     | 𝑛   | 𝑛   |     |     |     |     |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
the value of 𝑖𝑡(𝛾𝑡) selects applies;
𝑛
• 𝜃𝑡 = 𝖱𝖾𝗅𝗈𝖺𝖽𝗂𝗇𝗀(−,−,𝜔 ) with target𝑡 ≠ 𝜔 : L-Raise applies if 𝑖𝑡(𝛾𝑡) raises, and other-
| 𝑛   |     | 𝑛   |     | 𝑛   | 𝑛   |     |     | 𝑛   |     |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
wise L-Divert does, landing that iteration rather than aborting it;
| 𝜃𝑡             | ) with target𝑡 |     |     |                        |     |     |     |     |     |
| -------------- | -------------- | --- | --- | ---------------------- | --- | --- | --- | --- | --- |
| • = 𝖠𝖼𝗍𝗂𝗏𝖾(−,𝜔 |                |     |     | ≠ 𝜔 : L-Leave applies. |     |     |     |     |     |
| 𝑛              | 𝑛              |     | 𝑛   | 𝑛                      |     |     |     |     |     |
Let no fiber be of any of these kinds, leaving some 𝑚  with 𝜃𝑡 = 𝖴𝗇𝗅𝗈𝖺𝖽𝗂𝗇𝗀(−,−,−). Construct
|     |     |     |     |     | 0   | 𝑚   |     |     |     |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
0
𝑚 ,𝑚 ,… as follows: given 𝑚  in 𝖴𝗇𝗅𝗈𝖺𝖽𝗂𝗇𝗀, either ¬relied𝑡 , in which case L-Unload applies
| 0 1 |     | 𝑗   |     |     |     | 𝑚   |     |     |     |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
𝑗
to 𝑚  and the construction stops, or there are 𝑚 ≠ 𝑚  and 𝑘  with installed𝑡  and
| 𝑗   |     |     |     |     | 𝑗+1 | 𝑗   |     | 𝑗   | 𝑚   |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
𝑗+1
𝜔𝑡 (𝑘 ) = 𝑚
| 𝑚 𝑗 𝑗 . In the latter case |     |     |     |     |     |     |     |     |     |
| -------------------------- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
𝑗+1
|     |     | 𝑘 ∈ | 𝑑   | ∩dom(𝜎𝑡 | ) ⊆ 𝑑 | ∩𝑝  |     |     |     |
| --- | --- | --- | --- | ------- | ----- | --- | --- | --- | --- |
|     |     | 𝑗   | 𝑚   |         | 𝑚     | 𝑚   | 𝑚   |     |     |
|     |     |     | 𝑗+1 |         | 𝑗     | 𝑗+1 | 𝑗   |     |     |
the second membership being Theorem 63(3) at the episode of 𝑚  that 𝑡 lies in, so that 𝑚 ≺
|     |     |     |     |     |     |     | 𝑗+1 |     | 𝑗   |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
𝑚 . Moreover target𝑡 ≠ 𝜔𝑡 : an 𝖴𝗇𝗅𝗈𝖺𝖽𝗂𝗇𝗀 fiber is outside the union defining 𝜎 , so 𝑘
| 𝑗+1 | 𝑚   | 𝑚   |     |     |     |     |     |     | 𝛾 𝑗   |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | ----- |
|     |     | 𝑗+1 | 𝑗+1 |     |     |     |     |     |       |
at 𝛾𝑡 is unprovided or provided by a fiber other than 𝑚 . Were 𝑚  in 𝖠𝖼𝗍𝗂𝗏𝖾 or 𝖱𝖾𝗅𝗈𝖺𝖽𝗂𝗇𝗀
|     |     |     |     |     |     | 𝑗   |     | 𝑗+1 |     |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
it would then be of one of the four kinds excluded, so it is in 𝖴𝗇𝗅𝗈𝖺𝖽𝗂𝗇𝗀 and the construction
continues. The 𝑚  are ≺-increasing, hence distinct by acyclicity, and dom(𝐹𝑡) is finite, so the
𝑗
construction stops.
Termination. Two claims bound 𝑆(𝑛).
(A) Over a maximal interval on which target𝑡 is constant at 𝜔∗, at most 𝐾
+4 steps act on 𝑛.
𝑛
Reading the 𝜃  columns of Table 1, from 𝖠𝖼𝗍𝗂𝗏𝖾(−,𝜔) with 𝜔 ≠ 𝜔∗ the fiber takes an L-Leave and
𝑛
an L-Unload and then, if 𝜔∗ ≠ ⊥, an L-Begin and at most len(𝑒 ) ≤ 𝐾 landings, plus a second
𝑛
...
L-Divert or L-Leave falls in the interval, the 𝜔 that the L-Begin writes being target𝑡 = 𝜔∗ itself,
𝑛
and at 𝖠𝖼𝗍𝗂𝗏𝖾(−,𝜔∗), at 𝖨𝗇𝖺𝖼𝗍𝗂𝗏𝖾(⊥) with 𝜔∗ = ⊥, and at 𝖨𝗇𝖺𝖼𝗍𝗂𝗏𝖾(𝜉) no rule applies at all.
(B) If target𝑡 ≠ target𝑡+1 and step 𝑡 acts on 𝑚, then either 𝑚 ≺ 𝑛 or step 𝑡 writes 𝜏 . By
| 𝑛   | 𝑛   |     |     |     |     |     |     |     | 𝑛   |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
Definition 46 the value of target  is a function of 𝜏  and of the tables of the providers of the
|     |     |     | 𝑛   |     | 𝑛   |     |     |     |     |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
keys of 𝑑 ; a provider satisfies 𝑘 ∈ dom(𝜎 )∩𝑑  and hence 𝑚 ≺ 𝑛, and a table changes only
| 𝑛   |     |     |     | 𝑚   | 𝑛   |     |     |     |     |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
target  either consumes a step of a fiber strictly ≺-below 𝑛 or is the one turn 𝜏  affords, so
| 𝑛   |     |     |     |     |     |     |     |     | 𝑛   |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
𝑉(𝑛) ≤ 1+∑ 𝑆(𝑚). Since ≺ is acyclic and 𝑁 is finite, the recursion
𝑚≺𝑛
|     |     | 𝐵(𝑛) | ≔   | (𝐾 +4)(2+ | ∑   | 𝐵(𝑚)) |     |     |     |
| --- | --- | ---- | --- | --------- | --- | ----- | --- | --- | --- |
𝑚≺𝑛
is well founded and defines 𝐵 with 𝑆(𝑛) ≤ 𝐵(𝑛); hence 𝑉(𝑛) is finite and ∑ 𝑆(𝑛) ≤ ∑ 𝐵(𝑛).
|                                                         |     |     |     |     |     |     |     | 𝑛   | 𝑛   |
| ------------------------------------------------------- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
one of its own, the registrations form a tree of bounded depth, and len(𝑒 ) ≤ 𝐾 bounds its
𝑛
...
whole is that its dynamic history leaves no trace: whatever sequence of activations and deacti-
vations a running system has been through, the state it quiesces at is the one the same insertions
...
composition, of the consistency with a from-scratch evaluation that change propagation estab-
lishes for incremental computation [45].
...
and every key it declares is provided by a supported fiber. The support relation on dom(𝐹 ) is
𝛾
...
at 𝛾:
𝑛 ∈ 𝐴 ≔ ¬𝜏 ∧(𝜋 = 𝗋𝗈𝗈𝗍∨𝜋 ∈ 𝐴)∧∀𝑘 ∈ 𝑑 . ∃𝑚 ∈ 𝐴. 𝑘 ∈ 𝑝 (63)
𝑛 𝑛 𝑛 𝑛 𝑚
where 𝜋 = 𝗋𝗈𝗈𝗍 marks a fiber the orchestrator inserted and 𝜋 otherwise the fiber whose
𝑛 𝑛
...
Proof. Order the names of dom(𝐹 ) by the index of the step that registered each, which Defin-
𝛾
ition 53 supplies by starting the sequence at an empty registry. The parent half of ⊲ descends
in that index: an O-Insert has 𝜋 ∈ dom(𝐹 ) as a premise, so a parent pointer names a fiber
𝛾
...
has 𝛾 ⊧ 𝑑 as a premise, so a fiber providing the key is 𝖠𝖼𝗍𝗂𝗏𝖾 already before it, and clause (2)
𝑚
of Definition 58 leaves the key no second possible provider. The fiber that would close the cycle
is therefore never registered, and the edge is absent from dom(𝐹 ). A well-founded recursion
𝛾
...
dom(𝜎 ), the keys its fibers have installed, and Definition 43 relates the two by dom(𝜎 ) ⊆
𝛾 𝑛
𝑝 alone. The support set therefore over-approximates the 𝖠𝖼𝗍𝗂𝗏𝖾 fibers in general, and the
𝑛
...
has installed every key of 𝑝, so that dom(𝜎 ) = 𝑝 at every 𝖠𝖼𝗍𝗂𝗏𝖾 fiber instantiating it.
𝑛 𝑛
...
𝑛 ∈ 𝐴′ ⟺ target (𝛾) ≠ ⊥
𝑛
By Definition 46 the right side holds exactly when ¬𝜏 and every 𝑘 ∈ 𝑑 lies in dom(𝜎 ), and
𝑛 𝑛 𝛾
dom(𝜎 ) = ⋃ 𝑝 by Definition 69. The middle clause is the one the target no longer carries,
𝛾 𝑚∈𝐴′ 𝑚
and registration supplies it: a fiber with 𝜋 ≠ 𝗋𝗈𝗈𝗍 is registered only by an activation of 𝜋 , and
𝑛 𝑛
if 𝜋 ∉ 𝐴′ then 𝜋 is not 𝖠𝖼𝗍𝗂𝗏𝖾, so its accumulator has run and retired 𝑛 by Definition 47, giving
𝑛 𝑛
𝜏 . Hence 𝐴′ satisfies the clauses of Definition 67, and Lemma 68 gives them one solution, so
𝑛
𝐴 = 𝐴′. □
Lemma 71. (Transposition.) Let the steps be pairwise independent and 𝐹𝑡 well formed, and
...
applicable at 𝛾𝑡, then step 𝑡 is applicable at the state step 𝑡+1 produces from 𝛾𝑡, and the
...
Proof. For (1), by Table 1 the step of 𝑚 writes 𝜃 and, within Ψ𝑡 ∈ 𝔐(𝑒 ), the table 𝜎 and
𝑚 𝑚 𝑚
the effect part. It therefore leaves 𝜃 and 𝑖 alone, and by the second condition of Definition 60
𝑛 𝑛
leaves the inverse and the continuation that 𝑖 yields alone as well, so only the premises of step
𝑛
𝑡+1 that mention target remain to be checked. Its retirement half cannot fall, no activation
𝑛
rule writing a 𝜏. Its resolution half cannot move either: step 𝑡+1 being applicable at 𝛾𝑡 puts
every 𝑘 ∈ 𝑑 in dom(𝜎𝑡), and clause (2) of Definition 58 makes the fiber providing such a 𝑘 the
𝑛
only one that can, so 𝑘 ∉ 𝑝 and no write of 𝜎 reaches a key of 𝑑 . The same argument in
𝑚 𝑚 𝑛
the other direction leaves step 𝑡 applicable. Finally Ψ𝑡 ∈ 𝔐(𝑒 ) and Ψ𝑡+1 ∈ 𝔐(𝑒 ) commute
𝑚 𝑛
...
For (2), the orchestration step has Ψ𝑡+1 = id by Table 1, so the two state maps commute
Γ
outright, and its edit𝑡+1 writes 𝜏 or dom(𝐹 ) at 𝑛 alone, which the activation step neither reads
𝑛 𝛾
nor writes: the premises of the latter read 𝜃 , 𝑖 , 𝜏 , and target , and an O-Insert of a fresh 𝑛
𝑚 𝑚 𝑚 𝑚
moves no target, a fresh fiber providing nothing, whereas an O-Retire or O-Remove of 𝑛 leaves
𝜎 where it was, 𝑛 being 𝖨𝗇𝖺𝖼𝗍𝗂𝗏𝖾 in the one case and unaffected in its table in the other. So
𝛾
...
Lemma 72. (Deletion.) Let the sequence of steps be pairwise independent, let every compo-
nent be total on its provision (Definition 69), let it reach a quiescent 𝛾𝑇 at which no fiber is
...
outside 𝑅.
Proof. The deleted steps leave the state where they found it. Let 𝑡 < ⋯ < 𝑡 be the steps of [𝑏,𝑢] that
1 𝑙
...
the deleted steps of 𝑛 write no field but 𝜃 , which Lemma 54(4) restores to 𝖨𝗇𝖺𝖼𝗍𝗂𝗏𝖾(⊥) at 𝑢, no
𝑛
...
No surviving step loses a premise. A step acting on 𝑚 ∉ 𝑅∪{𝑛} reads 𝑛 only through target (𝛾)
𝑚
or relied (𝛾). The first depends on 𝑛 when 𝑚 declares a key 𝑛 provides, hence 𝑛 ≺ 𝑚, and when
𝑚
𝑛 registered 𝑚, which puts 𝑚 ∈ 𝑅. In the first case 𝑚’s episode does not close, by hypothesis,
so it is open at 𝛾𝑇, where quiet gives 𝜔 = target𝑇 and Lemma 70 puts its values among the
𝑚 𝑚
𝖠𝖼𝗍𝗂𝗏𝖾 fibers, which 𝑛 is not; since a key has at most one possible provider, 𝑛 provided no key
of 𝑑 at 𝑚’s L-Begin either. The second reads 𝑛 only through the values of 𝜔 , and deleting the
𝑚 𝑛
...
from 𝛾0 by a sequence that takes the same orchestration steps in their original order,
...
𝑛 ,…,𝑛 of 𝐴 linearizing ⊲, one episode of each 𝑛 in that order.
1 𝑘 𝑖
2. (Confluence.) Any two such sequences from 𝛾0 taking the same orchestration steps reach
...
Episodes are sorted and made contiguous, by induction on |𝐴|. Let 𝑛 be ⊲-minimal in 𝐴. Then 𝑑 =
1 𝑛
1
⌀ and 𝜋 = 𝗋𝗈𝗈𝗍, since Definition 67 puts a provider of a key of 𝑑 and the fiber registering 𝑛
𝑛 𝑛 1
1 1
in 𝐴 while ⊲ puts both below 𝑛 . So target reads no field of another fiber and, no orchestration
1 𝑛
1
step remaining to write 𝜏 and no fiber below 𝑛 remaining to retire it, is constant. Every
𝑛 1
1
step acting on 𝑛 is an activation step, no episode closing, and its remaining premises read
1
𝜃 and 𝑖 , which by Table 1 only 𝑛 writes; each is therefore applicable at every earlier state,
𝑛 𝑛 1
1 1
and Lemma 71 moves it one place earlier without moving the endpoint. The number of steps
of other fibers preceding a step of 𝑛 drops by one at each application, so the episode of 𝑛
1 1
becomes an initial contiguous block. The argument repeats on 𝐴∖{𝑛 } over the suffix that
1
follows the block, where 𝑛 is 𝖠𝖼𝗍𝗂𝗏𝖾 throughout and takes no further step, so it too contributes
1
...
states then differ in that fiber’s lifecycle state. They do not differ in anything else, by Corol-
lary 62, which puts a failed fiber’s contribution to the state at nothing.
In the base calculus of Section 4.2 the same theorem holds, and the proof needs no substi-
tution beyond dropping one clause. L-Unload carries no guard there, so the last paragraph of
Lemma 72 is vacuous; the rest of that lemma appeals to quiet𝑇 alone, which the base calculus
supplies unchanged.
The theorem is what licenses reasoning about a Cordis application as though it were stati-
cally assembled. An orchestrator that adds a component, removes it, replaces a provider, and
...
prescribes no concrete scenario; its sole responsibility is to supply universal dynamic compo-
sition semantics. The implementation is layered into three tiers: (1) the core library (Section 5.1)
implements the effect and coeffect systems directly; (2) the component loader (Section 5.2)
extends the core with configuration reconciliation and hot module replacement; and (3) appli-
cation frameworks such as Koishi (Section 5.3) build domain-specific functionality on top of
...
Γ
∞
ctx, the first-class context
𝛾 ∈Γ the context tree together with everything the running system
has touched
𝔈 , 𝔈iter Effect callback returning / yielding inverses
Γ Γ
effect (𝑒) ctx.effect(callback)
Γ
...
⟨𝑑,𝑝,𝑒,𝜋,𝜎,𝜏,𝜃⟩ fiber, the instantiation of a component in ℭ
Γ
dom(𝐹 𝛾 ) enumerated through ctx.registry
𝑛:𝔑 fiber.uid
𝑑 :𝔇 fiber.inject
Γ
𝑝:𝔓 Γ the component’s provide
𝑒:𝔈∗ fiber.apply
Γ
...
provider 𝑘 (𝛾) an Impl whose provider fiber is ACTIVE
...
through the context is automatically tracked and recovered upon component unloading. Oper-
ationally, ctx.effect is the realization of effectiter (Definition 52): it takes a callback of type 𝔈iter
Γ Γ
and lifts it to 𝔈iter, yielding a dispose closure that, when invoked, recovers the effect. Cordis
𝜕Γ
accepts both 𝔈 and 𝔈iter through this one operation (ad-hoc polymorphism); we take the
Γ Γ
iterator form as representative, since a plain effect function is the degenerate iterator that yields
a single inverse. What the operation does not check is the witness that 𝔈∗ carries: the callback
Γ
...
18 return dispose
The engine execute drives the callback as an effect iterator (𝔈iter, Definition 51) and folds
Γ
...
• @@store: the value store 𝜎 : (𝑟 : 𝑅) ⇀ 𝒱︀ from realm symbols to typed values;
𝑟
• @@isolate: the realm table 𝜌 : Map(𝐾,𝑅) from coeffect keys to realm symbols;
• @@intercept: the interception table 𝜄 : (𝑘 : 𝐾) → ℳ︀ assigning each key its metadata.
𝑘
...
Provision and notification. Since set(𝑘,𝑣) has type 𝔈 (Section 3.1), coeffect provision is
Σ
...
13 target 0 ← fiber.target
14 fiber.committed ← resolve(fiber.inject) ▷ commit the view
15 recover ← await execute(fiber.apply, () ↦ fiber.target = target 0 )
16 fiber.dispose ← recover ∘ fiber.dispose
17 if fiber.target = target 0 then
18 fiber.state ← ACTIVE
2create_task schedules an async function to run concurrently and returns a handle to it (stored in fiber.inertia).
...
The core library equips component developers with imperative primitives for dynamic compo-
sition, such as ctx.effect, ctx.use, and ctx.set. A separate concern arises for application
orchestrators, who assemble pre-existing components into a running system and adjust the
composition over its lifetime. The component loader addresses this concern by introducing a de-
clarative configuration layer: the orchestrator specifies the desired composition as a persistent
...
• Theorem 73 makes the quiescent state a function of the final configuration alone: what-
ever instantiations and retirements the loader performs on the way, and in whatever
...
• Theorem 63 lets the entries be instantiated together, with no load order for the orches-
trator to arrange: a fiber whose declared keys are not yet provided waits at its L-Begin, and
...
provider. The loader answers it with delimiters: one symbol 𝛿 per key, under which each context
𝑘
...
5 for 𝑘 in Δ do
6 entry.ctx[𝛿 ] ← fresh tag
𝑘
7 diff[𝑘] ← (𝜌(𝑘), 𝜌′(𝑘), entry.ctx[𝛿 ], store[𝜌(𝑘)].fiber.ctx[𝛿 ])
𝑘 𝑘
...
| --- | ---------- | --------------- | --- | --- | --- | --- | --- |
|     | 1 2        | 1 2             |     |     |     |     |     |
12 if 𝑑 =𝑑  and store[𝑠 ] and not store[𝑠 ] then ▷ the binding is the entry’s own
|     | 1              | 2           | 1   | 2   |     |     |     |
| --- | -------------- | ----------- | --- | --- | --- | --- | --- |
| 13  | store[𝑠        | ] ← store[𝑠 | ]   |     |     |     |     |
|     |                | 2           | 1   |     |     |     |     |
...
| --- | ---------- | --------------- | --- | --- | --- | --- | --- |
| 16  | 1 2        | 1 2             |     |     |     |     |     |
return fiber.ctx[@@isolate][𝑘] ∈ {𝑠 , 𝑠 } and (fiber.ctx[𝛿 ] = 𝑑 ) ≠ (𝑑 =𝑑
| 17  |     |     | 1   | 2   | 𝑘 1 | 2 1 ) |     |
| --- | --- | --- | --- | --- | --- | ----- | --- |
notify(entry.ctx, Δ, affected) ▷ in place of the realm test of Algorithm 3
18
The test turns on one property of delimiters. The tag under 𝛿
𝑘  is written on the entry’s
context and inherited by every context derived from it, and it is drawn afresh at each reassign-
ment, so for a context 𝛾′
|     | 𝛾′[𝛿 | ] = 𝑑 | ⟺ 𝛾′ is derived from the entry's context |     |     |     | (65) |
| --- | ---- | ----- | ---------------------------------------- | --- | --- | --- | ---- |
𝑘 1
Write own(𝛾′) for that condition, of which 𝑑
= 𝑑  is the instance at the provider. The
2 1
| reassignment moves the contexts satisfying own from 𝑠 |     |     |     |  to 𝑠 |     |     |     |
| ----------------------------------------------------- | --- | --- | --- | ----- | --- | --- | --- |
 and leaves the others where they
|     |     |     |     | 1 2 |     |     |     |
| --- | --- | --- | --- | --- | --- | --- | --- |
are, and by the loop above it moves the binding to 𝑠  exactly when the provider satisfies own.
2
...
the accepted modules’ caches3, backing up each removed module to enable rollback, then re-
...
Koishi is an open-source chatbot application framework built on Cordis4. Over four years of
development, it has accumulated over 4000 community-contributed plugins5, ranging from
...
Because effects performed through the context are tracked and their inverses composed auto-
matically (Section 3.1), even an inexperienced author obtains ordered cleanup for a plugin’s
...
Threats to validity. The evidence here is drawn from a single ecosystem in a single host lan-
guage, so it cannot separate the merits of the paradigm from those of its TypeScript realization
...
extends to broader engineering concerns, and discusses the design tensions and open prob-
lems.
6.1. System Boundary
...
location lies outside when either ability fails, so an operation on it acts as id and is therefore
Γ
...
supply an inverse for, so operations that acted as id come to be tracked in Γ and recovered. The
Γ
67
...
hands to the file or the datagram a send puts on the wire, and the push acts as id , leaving the
Γ
...
[53]. Each process hosts its own Cordis context with local providers; a coordinating compo-
nent links them, treating each as a remote provider. Cross-process service access is mediated
...
Sandboxing untrusted components. When a component’s code cannot be trusted, lan-
guage-level access control is insufficient, since a malicious component with access to the host
...
fault isolation [57], a separate language runtime, a sandboxed process, or a virtualized con-
tainer [58]. Whatever the mechanism, the untrusted component runs in its own sandboxed
...
make each component depend on the other. However, the two interaction directions are logi-
cally independent concerns. Decomposing them yields four components: server-core, access-
...
integration components can grow quadratically with 𝑛, since each pair of interacting compo-
nents may require a distinct component for each direction of interaction. This does not affect
...
into a single installable unit), convention-based wiring (i.e., automatically connecting compo-
nents whose names or types match a pattern), and scaffold tooling (i.e., generating boilerplate
...
𝒱︀ ensures type-level agreement within a single compilation unit, but this guarantee breaks
𝑘
...
changing method signatures, altering behavioral contracts) between versions, while a con-
sumer compiled against an earlier interface continues to declare the same key 𝑘. The
...
dependencies are semantically peer dependencies: a component does not bundle its dependen-
cies internally but expects the runtime context to supply them. Package managers with peer
...
Structural compatibility. A fully language-agnostic approach would replace the member-
ship check 𝑘 ∈ dom(𝜎) with a compatibility predicate that verifies the provider’s actual
interface structurally subsumes the consumer’s expectation. This is analogous to structural
subtyping [73]: a provider satisfies a consumer if the provided interface is a subtype of the re-
quired interface. The challenge lies in defining this predicate language-agnostically: structural
...
existing general-purpose languages, representing them as monadic values that a runtime exe-
cutes. ZIO in Scala [82] models a computation as ZIO[R,E,A] and Effect-TS in TypeScript [83]
...
Algebraic effects as capabilities. Algebraic effects (Section 2.1) make effect operations visi-
ble to the type system. The extension closest to our work is Brachthäuser et al.‘s Effekt language,
which reinterprets effect types as capabilities [85, 86]: an effect type expresses what a compu-
tation requires from its context rather than what side effects it may produce. This perspective,
...
types as an umbrella notion encompassing both effect reasoning (via graded monads) and co-
effect reasoning (via graded comonads), realized in the Granule language, demonstrating that
...
execution context, so that behavior adapts without the base code naming its context depen-
dencies [94]. COP and Cordis coincide in treating context as a first-class, runtime-mutable
...
scatter across components, but its analogue of an aspect is a coeffect: a shared point of media-
tion many components declare a dependence on, so that crosscutting behavior can be reshaped
...
model; dynamic-AOP systems [97, 98] can also weave and unweave at runtime, but as a stand-
alone operation, neither bound to a component’s lifecycle nor triggering re-resolution among
the advised code.
...
handing state forward through the module.hot or import.meta.hot API across a reload. Com-
pared with Cordis’s module replacement (Section 5.2), these approaches migrate in-memory
...
group of memory operations either commits or aborts, rolling memory back to its pre-trans-
action state. Reversible computing, from Landauer and Bennett’s thermodynamic analyses
...
follows from a record the runtime maintains rather than from cleanup the developer remem-
bers to write, which makes this family the closest systems-level precedent for revertible effects.
...
Initialization-time dependency wiring. Two established mechanisms wire components to-
gether at initialization time. Dependency injection frameworks [38] (e.g., Spring [117], Guice,
...
tree. Some support dynamic scoping (e.g., Spring’s prototype/request scopes, Angular’s hier-
archical injectors), but neither re-resolves reactively: when a provider is replaced or removed at
...
declare provided and required services, with the runtime automatically activating and deacti-
vating them as services appear and disappear; iPOJO’s Gravity project [119] explicitly targets
...
Value-level reactivity. Functional reactive programming (FRP) [120] and its modern incar-
nations (e.g., signals [121, 122] in SolidJS, Vue’s reactivity system, Angular Signals) propagate
change at a value-level granularity: when a signal changes, derived computations are re-evalu-
ated synchronously or under a scheduler [123]. Cordis’s reactive coeffects act at a component-
...
supplies the effects with independence, constituting a programming paradigm for spatiotem-
poral composability. Combining these mechanisms into the notion of a component then gives
a calculus of dynamic composition, whose metatheory carries spatiotemporal composability
from a single component to a whole system of interleaved components. We realize this para-
digm as the Cordis meta-framework, with a core library providing effect tracking and coeffect
...
References
[1] D. L. Parnas, “On the criteria to be used in decomposing systems into modules,” Com-
munications of the ACM, vol. 15, no. 12, pp. 1053–1058, 1972, doi: 10.1145/361598.361623.
[2] D. Birsan, “On Plug-ins and Extensible Architectures,” ACM Queue, vol. 3, no. 2, pp. 40–
46, 2005, doi: 10.1145/1053331.1053345.
[3] B. Burns, B. Grant, D. Oppenheimer, E. Brewer, and J. Wilkes, “Borg, Omega, and Kuber-
netes,” Communications of the ACM, vol. 59, no. 5, pp. 50–57, 2016, doi: 10.1145/2890784.
...
Wesley, 2002.
[8] R. Lopopolo, “Harness Engineering: Leveraging Codex in an Agent-First World.” [On-
line]. Available: https://openai.com/index/harness-engineering/
...
running-apps
[10] L. Wang et al., “A Survey on Large Language Model Based Autonomous Agents,” Fron-
tiers of Computer Science, vol. 18, no. 6, p. 186345, 2024, doi: 10.1007/s11704-024-40231-1.
...
Conference on Functional Programming, in ICFP 2016. Nara, Japan: Association for Com-
puting Machinery, 2016, pp. 476–489. doi: 10.1145/2951913.2951939.
...
Software Science and Computation Structures, Berlin, Heidelberg: Springer Berlin Heidel-
berg, 2002, pp. 342–356. doi: 10.1007/3-540-45931-6_24.
...
j.entcs.2015.12.003.
[27] D. Leijen, “Koka: Programming with Row Polymorphic Effect Types,” Electronic Proceed-
ings in Theoretical Computer Science, vol. 153, pp. 100–126, Jun. 2014, doi: 10.4204/
eptcs.153.8.
...
[32] T. Uustalu and V. Vene, “Comonadic Notions of Computation,” Electronic Notes in Theo-
retical Computer Science, vol. 203, no. 5, pp. 263–284, 2008, doi: 10.1016/j.entcs.2008.05.029.
...
[43] R. P. James and A. Sabry, “Yield: Mainstream Delimited Continuations,” in First Interna-
tional Workshop on the Theory and Practice of Delimited Continuations (TPDC 2011), 2011,
...
[51] J. Kramer and J. Magee, “The Evolving Philosophers Problem: Dynamic Change Man-
agement,” IEEE Transactions on Software Engineering, vol. 16, no. 11, pp. 1293–1306, 1990,
...
[56] R. N. M. Watson, J. Anderson, B. Laurie, and K. Kennaway, “Capsicum: Practical Capa-
bilities for UNIX,” in Proceedings of the 19th USENIX Security Symposium, 2010, pp. 29–46.
...
SLA), 2004, pp. 331–344. doi: 10.1145/1028976.1029004.
...
[83] Effect, “Effect: A TypeScript library for building robust applications.” [Online]. Avail-
able: https://effect.website/
...
[90] R. Bianchini, F. Dagnino, P. Giannini, and E. Zucca, “A Java-like calculus with heteroge-
neous coeffects,” Theoretical Computer Science, vol. 971, p. 114063, 2023, doi: https://doi.
...
[97] A. Popovici, T. Gross, and G. Alonso, “Dynamic Weaving for Aspect-Oriented Program-
ming,” in Proceedings of the 1st International Conference on Aspect-Oriented Software
...
teenth Annual ACM Symposium on Principles of Distributed Computing, in PODC '95. 1995,
pp. 204–213. doi: 10.1145/224964.224987.
[108] T. Harris, S. Marlow, S. Peyton Jones, and M. Herlihy, “Composable Memory Transac-
tions,” in Proceedings of the Tenth ACM SIGPLAN Symposium on Principles and Practice of
...
Self-Interpreter,” in Proceedings of the 2007 ACM SIGPLAN Workshop on Partial Evalua-
tion and Semantics-Based Program Manipulation, in PEPM '07. 2007, pp. 144–153. doi:
...
nent Framework,” in IEEE International Conference on Services Computing, 2007, pp. 474–
...
ming: The Cost of Consistency,” IEEE Trans. Software Eng., vol. 44, no. 7, pp. 689–711,
